...
(async () => {
  ...
  // Scoped to the ticket board, like the width checks above. The page emits one
// of these per board and hides the ones with nothing to page - the sow and
// epic backlogs say "Show 0 more of 0" and are hidden - so an unscoped
// selector matches three and Playwright refuses it.
const visibleCards = column => page.locator(`.board--ticket [data-column="${column}"] > li:not([hidden])`).count();
  const backlogTotal = await page.locator('.board--ticket [data-column="backlog"] > li').count();
  ...
  const moreLabel = await page.locator('.board--ticket [data-more-for="backlog"]').textContent();
  if (!/Show \d+ more of \d+/.test(moreLabel))
    throw new Error(`the reveal button must say how many remain, got "${moreLabel}"`);
  if (await page.locator('.board--ticket [data-more-for="planning"]').evaluate(node => node.hidden) !== true)
    throw new Error('a column with nothing hidden must not offer to show more');
  await page.locator('.board--ticket [data-more-for="backlog"]').click();
  ...
  const badge = await page.locator('.board--ticket [data-count-for="backlog"]').textContent();
