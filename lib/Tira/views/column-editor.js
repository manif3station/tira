...
const columnRow = (column, allNames) => {
  ...
  row.append(grip, label, minutes, eye, entryWrap);
...
const columnLayout = () =>
  Array.from(columnList.querySelectorAll(".column-row")).map((row) => {
    ...
    const entry = {
      ...
      label:
        row.querySelector(".column-row__label").value.trim() ||
        row.dataset.name,
      watched:
...
const addColumn = () => {
  ...
  if (columnList.querySelector('[data-name="' + slug + '"]')) {
  ...
  const allNames = Array.from(rows).map((r) => r.dataset.name);
