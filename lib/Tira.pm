# Documentation lives in the sibling lib/Tira.pod (TKT-1098) - perldoc Tira
# finds it there automatically, the standard CPAN same-basename convention.
package Tira;
...
# The notification/warning subsystem (card-reminder escalation and the
# board's own warning log) lives in lib/Tira/Notification.pm now (TKT-1092's
# decomposition umbrella, TKT-1102's own lift), required lazily by each
# forwarder below - a command that never touches a reminder or a warning
# never compiles it. See that module for the full account of what moved and
# why.
# _duration_phrase is the one bareword (non-method) call into this concern
# from outside it - t/48-escalation.t calls it directly, not through $self.
sub _duration_phrase { require Tira::Notification; return Tira::Notification::_duration_phrase(@_) }
sub clearance_list      { my $self = shift; require Tira::Notification; return Tira::Notification::clearance_list( $self, @_ ) }
sub notification_message { my $self = shift; require Tira::Notification; return Tira::Notification::notification_message( $self, @_ ) }
sub warning_list         { my $self = shift; require Tira::Notification; return Tira::Notification::warning_list( $self, @_ ) }
sub warning_add          { my $self = shift; require Tira::Notification; return Tira::Notification::warning_add( $self, @_ ) }
...
sub warning_clear        { my $self = shift; require Tira::Notification; return Tira::Notification::warning_clear( $self, @_ ) }

# The escalation level is derived, never stored on the card. One row per
# delivered notification; the level is how many rows that card already has
# in the column it is sitting in, so a move resets escalation for free.
# _sqlite_available STAYS HERE rather than moving with the rest of this
# concern to lib/Tira/Notification.pm - _search_index_dbh, an unrelated
# concern, calls it too, so it is shared, generic infrastructure rather than
# this concern's own. lib/Tira/Notification.pm calls it fully qualified,
# Tira::_sqlite_available(), the same way it calls Tira::json_object() and
# Tira::_card_unblocked_at() for the other two things it still reaches back
# here for.
sub _sqlite_available {
    return eval { require DBI; require DBD::SQLite; 1 } ? 1 : 0;
}

sub notification_record  { my $self = shift; require Tira::Notification; return Tira::Notification::notification_record( $self, @_ ) }
sub notification_level   { my $self = shift; require Tira::Notification; return Tira::Notification::notification_level( $self, @_ ) }
sub notification_list    { my $self = shift; require Tira::Notification; return Tira::Notification::notification_list( $self, @_ ) }
