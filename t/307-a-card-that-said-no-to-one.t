...
my ($note) = grep { $_->{item} eq 'left a note' } @{ $tira->checklist_list( project => $root, ref => $card->{ref} ) };
$tira->checklist_update( project => $root, ref => $card->{ref}, id => $note->{id}, status => 'done' );
...
my ($sibling_note) = grep { $_->{item} eq 'left a note' } @{ $tira->checklist_list( project => $root, ref => $sibling->{ref} ) };
$tira->checklist_update( project => $root, ref => $sibling->{ref}, id => $sibling_note->{id}, status => 'done' );
...
my ($n1) = grep { $_->{item} eq 'left a note' } @{ $tira->checklist_list( project => $root, ref => $extra->{ref} ) };
my ($n2) = grep { $_->{item} eq 'said why' } @{ $tira->checklist_list( project => $root, ref => $extra->{ref} ) };
$tira->checklist_update( project => $root, ref => $extra->{ref}, id => $n1->{id}, status => 'done' );
$tira->checklist_update( project => $root, ref => $extra->{ref}, id => $n2->{id}, status => 'done' );
