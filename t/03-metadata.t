...
close $env;
like( $env_text, qr/^VERSION=0\.83$/m, '.env stores version 0.83' );
...
is( scalar @use_cases, 105, 'SKILLS.md contains exactly 105 numbered use cases' );
my %seen;
while ( $skills_text =~ /^### UC-(\d{3}):/mg ) {
    $seen{$1}++;
}
is_deeply( [ sort keys %seen ], [ map { sprintf '%03d', $_ } 1 .. 105 ], 'use cases are numbered UC-001 through UC-105' );
...
use lib 'lib';
use Tira;
is( $Tira::VERSION, '0.83', 'module version matches .env' );
