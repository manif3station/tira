...
sub _dashboard_table {
        ...
        my $cells = join '', map {
            ...
            my $cards = join '', map {
                ...
                '<li data-ref="' . $ref . '" data-mtime="' . $mtime
                  . '"><button class="card" draggable="true" type="button" data-ref="' . $ref
    ...
    my $card_binding = $args{live}
      ? q{card.draggable=true;card.onclick=()=>{const ref=card.dataset.ref;const type=card.closest(".board").dataset.type;fetch("/record?type="+encodeURIComponent(type)+"&ref="+encodeURIComponent(ref),{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("detail failed");return response.json()}).then(showCard).catch(()=>{});};}
    ...
    my $drag_script = $args{live}
      ? q{document.addEventListener("dragstart",event=>{const card=event.target.closest(".card");if(card)event.dataTransfer.setData("text/plain",card.dataset.ref)});document.addEventListener("dragover",event=>{if(event.target.closest(".cards"))event.preventDefault()});document.addEventListener("drop",event=>{const list=event.target.closest(".cards");const cardRef=event.dataTransfer.getData("text/plain");if(!list||!cardRef)return;event.preventDefault();const board=list.closest(".board");fetch("/move",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:board.dataset.type,ref:cardRef,column:list.dataset.column})}).then(response=>{if(!response.ok)throw new Error("move failed");return response.json()}).then(()=>refreshDashboard()).catch(()=>{})});}
      : '';
