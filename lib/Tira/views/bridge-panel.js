...
if (bridgeSection) {
  ...
  const line = (entry, index) => {
    ...
    // TKT-1024, his words: "a flip flop colour strip to determine line by
    // line at first glance" - a dense dump of violation lines had no visual
    // rhythm to scan against.
    item.className = "bridge-line " + (index % 2 === 0 ? "bridge-line--even" : "bridge-line--odd");
  ...
  const paintBridge = (payload) => {
    ...
    bridgeList.textContent = "";
    newestFirst.forEach((entry, index) => bridgeList.appendChild(line(entry, index)));
