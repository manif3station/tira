...
- `tira.comment.update --ref REF --comment ID (--text TEXT|--file FILE) [--format markdown|text] [-o FORMAT]` - the updated comment also carries `text` alongside `body`, same as `comment.add`. An unknown `--comment` id refuses the same way as `comment.remove`. TKT-491. **Since TKT-692, a missing `--comment` refuses as missing, not as unknown** - it used to fall into the not-found lookup with an undef id, leaking a warning and reporting that empty-string "id" was not found rather than that none was given.
