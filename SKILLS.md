...
**The card it raises arrives incomplete, and that is a known cost rather than a surprise** (observed on TKT-948, 2026-09-05). The gate writes a title, a description sliced from the Changes file, and a checklist; it writes no problem statement, no scope, no acceptance criteria and no parent. Measured on the 5.77 to 5.83 upgrade, `d2 tira.ticket.missing` reported eleven empty fields on the card the gate had just created, and police reported it as an orphan within the minute. Because `tools/card-holes` refuses a push while any live card is incomplete, every upgrade adds one release-gate violation that somebody has to clear by hand before the next push - on a board that already had 29 incomplete cards that evening, the gate's own card would have been the thirtieth. Nothing is wrong with the raise itself; the card is real work and belongs on the board. What is missing is that the gate knows the version range and holds the changelog entries between them, so it could derive the fields it leaves empty. Whether to fill them, exempt a gate-raised card from `card-holes`, or announce instead of creating is TKT-956.
