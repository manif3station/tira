...
sub providers {
    ...
    return (
        ...
        # The job-provider block (jobs, job_run, job_save, job_delete, job_stop,
        # job_start, job_check) is lifted into Tira::CLI::Browser::Jobs -
        # TKT-1042, the same day TKT-1041 lifted Tira::CLI.pm's own move-path
        # guards for the same reason (t/524's 500-line limit). Called, not
        # forwarded: nothing outside this sub ever named these closures, they
        # only existed as values in this hash literal.
        Tira::CLI::Browser::Jobs::job_providers( $tira, $project, $json ),
