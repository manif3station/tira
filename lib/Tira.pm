...
# The tasklist commands and their private _tasklist_* helpers live in
# lib/Tira/Tasklist.pm now (TKT-746's second lift, TKT-832), required lazily
# by each forwarder below - a command that never touches the tasklist never
# compiles it. See that module for the full account of what moved and why.
sub tasklist_list                    { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_list( $self, @_ ) }
sub tasklist_sessions                { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_sessions( $self, @_ ) }
sub tasklist_add                     { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_add( $self, @_ ) }
sub tasklist_next                    { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_next( $self, @_ ) }
sub tasklist_shift                   { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_shift( $self, @_ ) }
sub tasklist_pop                     { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_pop( $self, @_ ) }
sub tasklist_unshift                 { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_unshift( $self, @_ ) }
sub tasklist_slice                   { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_slice( $self, @_ ) }
sub tasklist_remove                  { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_remove( $self, @_ ) }
sub tasklist_import                  { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_import( $self, @_ ) }
sub tasklist_update                  { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_update( $self, @_ ) }
sub tasklist_prune                   { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_prune( $self, @_ ) }
sub tasklist_task_attach_add         { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_task_attach_add( $self, @_ ) }
sub tasklist_task_attach_add_content { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_task_attach_add_content( $self, @_ ) }
sub tasklist_task_attach_discard     { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_task_attach_discard( $self, @_ ) }
sub tasklist_task_ref_link           { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_task_ref_link( $self, @_ ) }
sub tasklist_task_ref_unlink         { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::tasklist_task_ref_unlink( $self, @_ ) }

# _tasklist_read is called as a method from outside the tasklist concern too
# (search, police_pass) - kept answering here the same way.
sub _tasklist_read { my $self = shift; require Tira::Tasklist; return Tira::Tasklist::_tasklist_read( $self, @_ ) }
...
# Attachments live in Tira::Attachment since TKT-746. The names stay here so
# nothing that called them breaks, and the module is required at the CALL SITE
# so a command that never touches an attachment never compiles it.
#
# The two underscore names are here for the same reason: question_attach,
# question_voice and _backfill_added_at call them, because questions attach
# files through the attachment store.
sub attachment_add         { my $self = shift; require Tira::Attachment; return Tira::Attachment::attachment_add( $self, @_ ) }
sub attachment_add_content { my $self = shift; require Tira::Attachment; return Tira::Attachment::attachment_add_content( $self, @_ ) }
sub attachment_detach      { my $self = shift; require Tira::Attachment; return Tira::Attachment::attachment_detach( $self, @_ ) }
sub attachment_discard     { my $self = shift; require Tira::Attachment; return Tira::Attachment::attachment_discard( $self, @_ ) }
...
# The View's files, read from beside this module rather than from wherever the
# process happened to start. An installed skill is run from anywhere, so a
# relative path would find the templates only when the board was served from
# the source tree - the same problem tools/card-holes solved for its
# entrypoints.
#
# Read at render and inlined into the page, never linked. The board makes no
# network requests, and a <link> or a <script src> pointing at a served asset
# would be one: what changes here is where the bytes LIVE, not how they reach
# the browser.
#
# Kept as plain files rather than as strings in this module because a string in
# Perl carries an escaping layer that the browser never sees. TKT-645 shipped a
# dead dashboard through exactly that gap - a backslash pair inside a q{}
# collapsed to one, and the served script had a quote that escaped itself. A
# .css or .js file read verbatim cannot have that class of fault. TKT-703.
# The View proper. Template Toolkit renders the page from a template beside
# this module, rather than the module concatenating its own markup.
#
# INCLUDE_PATH is resolved from __FILE__ for the reason _view_asset explains:
# an installed skill is run from anywhere.
#
# The template chomps around every directive so it can be read on several lines
# while emitting exactly what the concatenation emitted - the page is one long
# line, and a readable template that added a newline per directive would change
# every byte after the first. That equivalence is not assumed: the card renders
# the board before and after and requires cmp to report no difference. TKT-703.
sub _render_view {
    my ( $name, $vars ) = @_;
    require Template;
    my $engine = Template->new( {
        INCLUDE_PATH => _view_dir(),
        ABSOLUTE     => 0,
        RELATIVE     => 0,
        ENCODING     => 'utf8',
    } ) or die 'Cannot start the template engine: ' . Template->error() . "\n";
    my $out = '';
    $engine->process( $name, $vars, \$out )
      or die "Cannot render '$name': " . $engine->error() . "\n";
    return $out;
}
...
sub _view_asset {
    ...
    open my $fh, '<:encoding(UTF-8)', $path
      or die "Cannot read view asset '$name' from $dir: $!\n";
    my $content = do { local $/; <$fh> };
    close $fh;
    return $content;
}
...
implemented in L<Tira::Tasklist> and reached through a one-line forwarder on
this class that C<require>s that module at the call site - so a command that
never touches the task list never compiles it. Names, arguments and return
values are unchanged by that move, and they stay documented here because here
is where callers still find them. TKT-832.
...
C<A comment needs some text>.

An B<empty> body is something this was alone in accepting: every sibling refuses
one - C<evidence_add> a summary, C<warning_add> a message, C<question_answer>
some text, C<checklist_add>/C<required_item_add> an item. On B<whitespace> the
siblings disagree, and this joins the stricter pair: C<warning_add> and
C<question_answer> test C</\S/>, while C<evidence_add>, C<checklist_add> and
C<required_item_add> test C<eq ''> and so still accept a value made of spaces.
...
Renders one Template Toolkit template from the View directory.

The dashboard had no view layer at all: Dancer2 served the board, Starman ran
it, and no template engine was configured, so the page was built by
concatenating Perl strings in this file. This is what replaced that. The page,
the sign-in page and the card and column dialogs each render through here.

C<INCLUDE_PATH> comes from C<_view_dir>, so an installed skill finds its
templates wherever it is run from - the reason is the one C<_view_asset> gives.
C<ABSOLUTE> and C<RELATIVE> are off, so a template name can only ever name a
file inside that directory.

The templates chomp around every directive. The page is one long line, and a
readable template that added a newline per directive would change every byte
after the first; chomping is what lets the markup be read on twenty lines and
still emit what the concatenation emitted. That equivalence is asserted rather
than hoped for - TKT-703 rendered the board before and after each of its five
moves and required C<cmp> to report no difference, which is how a stray script
fragment worth 216 bytes was caught in a 152KB page whose every element was
still correct.

A template that will not render dies rather than returning a partial page,
because a board that renders half its markup is harder to diagnose than one that
says which template failed. TKT-703.

=head2 _view_dir

Where the View lives: C<views> beside this module, from C<__FILE__>.

One place answers this so C<_view_asset>, C<_render_view> and
L<Tira::DashboardWeb>'s C<views> setting cannot disagree about it - three
readers of one question being the fault this board has spent a week finding in
other forms.

=head2 _view_asset

Reads one of the View's files from beside this module.

The dashboard had no view layer at all: its markup, stylesheet and front-end
were Perl strings in this file, 113,884 bytes of them on eight lines, the
longest 54,419. TKT-703 moves them out, and this is what the page calls to get
them back at render time.

The directory is resolved from C<__FILE__> rather than from the process's
working directory. An installed skill is run from anywhere, and a relative path
would find the assets only when the board happened to be served out of the
source tree - the same problem C<tools/card-holes> solved for its entrypoints.

Inlined into the page, never linked. The board makes no network requests, and a
C<< <link> >> or C<< <script src> >> pointing at a served asset would be one.
What this changes is where the bytes live, not how they reach the browser.

Files rather than strings, because a string in Perl carries an escaping layer
the browser never sees. TKT-645 shipped a dead dashboard through exactly that
gap: a backslash pair inside a C<q{}> collapsed to one and the served script
held a quote that escaped itself, so 98KB of JavaScript failed to parse while
every test still passed. A file read verbatim cannot have that fault.

A missing asset dies rather than rendering a page with no stylesheet, because a
board that looks broken is harder to diagnose than one that says what it could
not find.
