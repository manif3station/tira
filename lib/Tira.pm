...
sub _dashboard_table {
    ...
    my $refresh_action = $args{live}
      ...
      : q{location.reload()};
    my $live_helpers = $args{live} ? q{const recordsByRef=new Map();const showTitles=document.documentElement.dataset.withTitle==="1";const dialog=document.querySelector(".card-dialog");const showCard=record=>{if(!record)return;dialog.querySelector(".card-dialog__ref").textContent=record.ref||"Card";dialog.querySelector(".card-dialog__title").textContent=record.title||"Untitled";dialog.querySelector("pre").textContent=JSON.stringify(record,null,2);dialog.showModal()};dialog.querySelector(".card-dialog__close").addEventListener("click",()=>dialog.close());const buildCard=record=>{const item=document.createElement("li");item.dataset.ref=record.ref;item.dataset.mtime=String(Date.parse(record.updated_at||record.last_updated||"")||0);const button=document.createElement("button");button.className="card";button.type="button";button.dataset.ref=record.ref;const ref=document.createElement("span");ref.className="card__ref";ref.textContent=record.ref;button.appendChild(ref);if(showTitles){const title=document.createElement("span");title.className="card__title";title.textContent=record.title||"";button.appendChild(title)}item.appendChild(button);return item};const updateBoards=data=>{recordsByRef.clear();document.querySelectorAll(".board").forEach(board=>{const type=board.dataset.type;(data._column_order?.[type]||[]).forEach(column=>{const list=[...board.querySelectorAll(".cards")].find(node=>node.dataset.column===column);if(!list)return;const records=data[type]?.[column]||[];records.forEach(record=>recordsByRef.set(record.ref,record));list.replaceChildren(...records.map(buildCard))});sortBoard(board,document.documentElement.dataset.sort)});bindBoards()};}
      : '';
    ...
    my $dialog = $args{live}
      ? '<dialog class="card-dialog"><header><div><span class="card-dialog__ref">Card</span><h2 class="card-dialog__title">Details</h2></div><button class="card-dialog__close" type="button" aria-label="Close">&times;</button></header><pre></pre></dialog>'
      : '';
