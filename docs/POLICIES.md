...
Since 4.79, an item arriving is reported once and one disappearing is
reported once - only a board's first-ever pass stays quiet about an
existing tasklist, because every item there is a first sighting and
reporting all of them at once would be declined and never re-enabled. Each
finding settles the instant it has been seen once. TKT-606.
