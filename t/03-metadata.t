...
close $env;
like( $env_text, qr/^VERSION=0\.90$/m, '.env stores version 0.90' );
...
is( scalar @use_cases, 109, 'SKILLS.md contains exactly 109 numbered use cases' );
my %seen;
while ( $skills_text =~ /^### UC-(\d{3}):/mg ) {
    $seen{$1}++;
}
is_deeply( [ sort keys %seen ], [ map { sprintf '%03d', $_ } 1 .. 109 ], 'use cases are numbered UC-001 through UC-109' );
...
use lib 'lib';
use Tira;
is( $Tira::VERSION, '0.90', 'module version matches .env' );
