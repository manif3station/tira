...
# The move-path guards and their bookkeeping (chain order, exit/entry
# required actions, unjudged answers, the reminder and reset that follow a
# move) are lifted into Tira::CLI::Move - TKT-1041, mirroring TKT-607's own
# lift of the record verbs into Tira::CLI::Records. Documentation stays here:
# t/430 asserts "=head2 The four guards on the move path" is still in THIS
# file's own text, since a reader following the move path opens the index
# first, and the per-function =head2 entries below (_columns_for,
# _column_chain_violation, and so on) explain what each one below now only
# forwards to.
sub _columns_for {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_columns_for(@_);
}
sub _column_chain_violation {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_column_chain_violation(@_);
}
sub _populate_entry_required_actions {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_populate_entry_required_actions(@_);
}
sub _column_entry_required_action_violation {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_column_entry_required_action_violation(@_);
}
sub _unmet_in_column {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_unmet_in_column(@_);
}
sub _outstanding_here {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_outstanding_here(@_);
}
sub _column_required_action_violation {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_column_required_action_violation(@_);
}
...
sub _item_is_done {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_item_is_done(@_);
}
sub _remind_one_at_a_time {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_remind_one_at_a_time(@_);
}
sub _unjudged_answer_violation {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_unjudged_answer_violation(@_);
}
sub _first_line {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_first_line(@_);
}
sub _reset_linked_tasks_on_return {
    require Tira::CLI::Move;
    return Tira::CLI::Move::_reset_linked_tasks_on_return(@_);
}
