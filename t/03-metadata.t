...
close $env;
like( $env_text, qr/^VERSION=1\.35$/m, '.env stores version 1.35' );
...
is( scalar @use_cases, 130, 'SKILLS.md contains exactly 130 numbered use cases' );
my %seen;
while ( $skills_text =~ /^### UC-(\d{3}):/mg ) {
    $seen{$1}++;
}
is_deeply( [ sort keys %seen ], [ map { sprintf '%03d', $_ } 1 .. 130 ], 'use cases are numbered UC-001 through UC-130' );
...
use lib 'lib';
use Tira;
is( $Tira::VERSION, '1.35', 'module version matches .env' );
...
is( scalar @commands, 133, 'release ships exactly 133 executable CLI entrypoints' );
