...
my ( $bypass_status, $bypass_out, $bypass_err ) = run_cli(
    'export', undef, '--cache-ttl', '60', '--no-cache', '-o', 'json',
);
...
my ( $projected_status, $projected_out, $projected_err ) = run_cli(
    'export', undef, '--cache-ttl', '60', '--fields', 'ref', '-o', 'json',
);
...
my ( $ttl_status, undef, $ttl_err ) = run_cli(
    'export', undef, '--cache-ttl', '1', '-o', 'json',
);
...
run_cli( 'export', undef, '--cache-ttl', '60', '-o', 'json' );
...
( $status, undef, $err ) = run_cli(
    'export', undef, '--cache-ttl', '0', '-o', 'json',
);
