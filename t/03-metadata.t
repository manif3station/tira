...
close $env;
like( $env_text, qr/^VERSION=1\.42$/m, '.env stores version 1.42' );
...
is( scalar @use_cases, 134, 'SKILLS.md contains exactly 134 numbered use cases' );
my %seen;
while ( $skills_text =~ /^### UC-(\d{3}):/mg ) {
    $seen{$1}++;
}
is_deeply( [ sort keys %seen ], [ map { sprintf '%03d', $_ } 1 .. 134 ], 'use cases are numbered UC-001 through UC-134' );
...
use lib 'lib';
use Tira;
is( $Tira::VERSION, '1.42', 'module version matches .env' );
...
is( scalar @commands, 138, 'release ships exactly 138 executable CLI entrypoints' );
