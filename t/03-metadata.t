...
close $env;
like( $env_text, qr/^VERSION=0\.98$/m, '.env stores version 0.98' );
...
is( scalar @use_cases, 115, 'SKILLS.md contains exactly 115 numbered use cases' );
my %seen;
while ( $skills_text =~ /^### UC-(\d{3}):/mg ) {
    $seen{$1}++;
}
is_deeply( [ sort keys %seen ], [ map { sprintf '%03d', $_ } 1 .. 115 ], 'use cases are numbered UC-001 through UC-115' );
...
use lib 'lib';
use Tira;
is( $Tira::VERSION, '0.98', 'module version matches .env' );
