...
if (jobsSection) {
  ...
  const closeEditor = () => {
    ...
    if (open) {
      // A CARD THAT BECAME AN EDITOR HAS TO BECOME A CARD AGAIN, which is
      // worse than the old panel if it does not: the panel at least closed.
      // The card element is left in place and marked, so restoring it is
      // giving it back the contents the row builder made - and the next
      // reload rebuilds it anyway, thirty seconds later at the latest.
      const host = open.closest(".jobs-card");
      if (host) {
  ...
  // TKT-1030. What of the server's own job.recent tail has already been
  // folded into runLogs, per job id - so a later poll whose tail has moved
  // on can append only the new part rather than re-seeding the whole thing
  // (duplicate lines, freshness reset) or ignoring it entirely (the stale
  // panel this card is about).
  const seenRecent = new Map();
  ...
  const paintLog = (box, id) => {
    const kept = runLogs.get(id) || [];
    // The clear button is appended right after its log box (TKT-1020) - a
    // sibling lookup rather than a second parameter, so every existing
    // call site stays unchanged.
    const clearBtn = box.nextElementSibling;
    box.textContent = "";
    if (!kept.length) {
      box.hidden = true;
    ...
    const now = Date.now();
    // NEWEST FIRST, same construction the bridge panel uses (TKT-976):
    // slice() first since reverse() mutates in place. His report: a card
    // that looked dead was the newest line sitting past the fold, with a
    // force-scroll pulling every repaint back down to it anyway.
    const wasAt = box.scrollTop;
    // TKT-1024, his own live follow-up: "Also apply to the bridge log
    // section on the dashboard. not only on the job card" - the same
    // odd/even rhythm the bridge panel gets.
    kept.slice().reverse().forEach((entry, index) => {
      const span = document.createElement("span");
      span.className =
        "jobs-card__log-line" + (now - entry.at < FRESH_MS ? " is-fresh" : "")
        + (index % 2 === 0 ? " jobs-card__log-line--even" : " jobs-card__log-line--odd");
      // No trailing "\n": each span is now display:block (dashboard.css),
      // which already breaks the line - an appended newline under
      // white-space:pre-wrap would preserve as a visible blank line inside
      // that block. Caught by Codex review after the display:block fix.
  ...
  const openEditor = (job) => {
    ...
    // THE CARD ITSELF, since TKT-913. His words: "When I edit, Why a separated
    // card pop next to it as an editor and not on the actual card itself like
    // the task card?"
    //
    // THIS WAS DECIDED THE OTHER WAY ONCE AND HE HAS OVERRULED IT. TKT-880
    // raised it, TKT-892 answered - "too much to inline into a card row the
    // way a tasklist card edits itself, so it stays a panel" - and softened it
    // by opening the panel directly beneath its card. He has seen that and
    // asked the same question again, which is a rejection of the compromise.
    // The objection was about SIZE, and size is a layout problem rather than a
    // reason to keep two editing idioms on one page: the tasklist card grows
    // into its editor, and this one can too.
    //
    // The form itself is untouched - one form for create and edit, the
    // schedule-kind radio, the mode row hidden under Monitor, the loop row for
    // a command-mode monitor. Where it lives changed; what it is did not.
    ...
    let commandField = null;
    {
      ...
      // A TEXTAREA THAT GROWS, NOT A ONE-LINE INPUT. His report, with a
      // screenshot: "when the card has a long message. when i click on the
      // edit. instead of getting a textarea but i get a limited size of text
      // input instead? why? that isn't the same like the task card. If you
      // forget, look how task card do that." One control serves both modes,
      // and every message job on this board carries multi-sentence
      // instructions - editing JOB-003 showed "3-HOURLY DOC-ACCUR" and
      // scrolled the other 260 characters out of sight. TKT-951.
      //
      // rows = 1 so a command job is unchanged on screen: the box is one line
      // tall until its content needs more. That is the answer to the objection
      // recorded as Q-133 - a command is one line by design, and a tall empty
      // box invites pasting a shell script into a field that has no shell.
      commandField = document.createElement("textarea");
      ...
      // The task card's own helper, which is what "look how task card do that"
      // asks for - one implementation of growing-to-content rather than a
      // second one here. It grows on input and runs once for the value just
      // filled in above, so an existing long message opens at its full height
      // instead of at one line.
      growBox(commandField);
    }
    ...
    expectRow.textContent = "How often this monitor should speak, in minutes - tells a wedged monitor (process up, polling stopped) from one that is simply quiet. Leave blank for no expectation.";
    ...
    const applyKind = () => {
      ...
      // AND A LOOP IS FOR A MONITOR. The mode test below excludes a message
      // job because the engine refuses a loop there - "a message job announces
      // its text and runs nothing, so there is nothing to restart" - and the
      // engine refuses one on a CRON job in the very next breath, because it
      // fires on a tick rather than staying up. This line applied the first
      // rule and forgot its neighbour, so a cron job showed a checkbox and a
      // seconds field whose use the save rejected. TKT-911.
      //
      // All three terms are load-bearing: the kind rules out cron, and the
      // mode still rules out a cron message job, which is the case the kind
      // alone would let through.
      const looping = monitoring && modeCommand.checked && !modeMessage.checked;
    ...
    save.addEventListener("click", () => {
      // BEFORE THE ROUND TRIP, and against the ENGINE's own rule rather than
      // a second wording invented here. min="2" on the field is a measured
      // floor about the feeder's quiet window - below about two seconds a
      // healthy monitor reports no output and reads as dead - and is not
      // what Tira::Job::_job_fields actually enforces, which is only that
      // restart_every is a whole number of seconds greater than zero:
      //
      //   die "How long to wait before restarting is a whole number of
      //         seconds, greater than zero - '$restart' is not\n"
      //
      // Checked here so a pasted "2.5" or a scripted "0" is caught before
      // the save rather than after, and the message quotes that same rule
      // rather than inventing one that could drift from it. TKT-929.
      if (!loopRow.hidden && loopBox.checked && !/^[1-9][0-9]*$/.test(loopEvery.value)) {
        why.textContent =
          "How long to wait before restarting is a whole number of seconds, " +
          "greater than zero - '" + loopEvery.value + "' is not";
        why.hidden = false;
        return;
      }
      save.disabled = true;
      ...
      //
      // AND NULL RATHER THAN NOTHING WHEN THE ROW IS HIDDEN TOO, which these
      // two used to skip. job_update validates the job as it WOULD be, merging
      // any field the payload does not mention with what the record already
      // holds - deliberately, so an edit naming only the command cannot drop
      // how often a monitor said it would speak. So a monitor with an interval
      // switched to cron sent no interval, had its own merged back in, and was
      // refused for holding one: a save that fails over a control no longer on
      // screen. The expectation row had that fault from the day it was written
      // and nobody could turn a declared monitor into a cron job from this
      // page at all.
      //
      // Hidden means exactly "the engine refuses this field for this kind", so
      // null is the only legal value and clearing it cannot take away anything
      // that was allowed to stay. TKT-911.
      payload.restart_every = loopRow.hidden
        ? null
        : loopBox.checked
          ? Number(loopEvery.value)
          : null;
      payload.expect_every = expectRow.hidden
        ? null
        : expectField.value === ""
          ? null
          : Number(expectField.value);
      if (!creating) {
        payload.id = job.id;
      }
    ...
    if (owner) {
      // IN the card rather than beside it. The card's own contents are hidden
      // rather than removed, so restoring is putting them back rather than
      // rebuilding them - and the element keeps its identity, which is what
      // makes this one object instead of two.
      owner.classList.add("is-editing");
      Array.from(owner.children).forEach((child) => {
        child.hidden = true;
      });
      owner.appendChild(panel);
    } else {
      // Creating still appends to the section, because there is no card for a
      // job that does not exist yet - the one case where a form of its own is
      // the only thing it can be.
      jobsCards.appendChild(panel);
    }
    field.focus();
  };
  ...
  const jobRow = (job) => {
    ...
    if (Object.prototype.hasOwnProperty.call(job, "running")) {
      ...
      if (!Number.isFinite(spokeAt)) {
        ...
        // A declared expectation should be visible on the card whether or
        // not the monitor has gone stale - until now it only appeared once
        // silentFor had already outrun it, so a healthy monitor gave no
        // sign it had declared anything at all.
        beat.textContent = "Last spoke " + sinceWords(silentFor) +
          (job.expect_every ? ", expects every " + job.expect_every + " min" : "");
      }
      li.appendChild(beat);
    } else if (job.schedule_kind !== "monitor") {

      // THE SAME QUESTION, ASKED OF A CRON JOB. TKT-942. The branch above
      // belongs to an enabled monitor and reads last_output_at, which the
      // feeder stamps when a monitor calls in - so a cron job, of either
      // mode, showed nothing at all. The owner read that blank as proof his
      // jobs were broken and asked three times in one afternoon; they had
      // been firing on schedule the whole time, and the board simply never
      // said so.
      //
      // last_due_at comes from the police ledger, joined on at read by
      // job_list - never stored on the job, because the rule that knows the
      // instant must not write the record it judges. A disabled monitor
      // falls through both branches, which is right: it is never "due", so
      // there is no window here to report about it.
      const firedEl = document.createElement("div");
      firedEl.className = "jobs-card__beat";
      const firedAt = job.last_due_at ? Date.parse(job.last_due_at) : NaN;

      // AND WHETHER IT RAN, which is a different question from whether its
      // window came round. TKT-963, his report: he pressed Run now, the job
      // ran, and this line still said "Never fired" - because a manual run
      // never comes due and last_due_at is the only thing it read.
      //
      // last_run_at is stamped where the command is run, by both the button
      // and the schedule, so it answers for either. Preferred over the window
      // when both exist: "it ran four minutes ago" is what somebody testing a
      // job wants, and the window that carried it is the less interesting
      // half once something actually executed.
      const ranAt = job.last_run_at ? Date.parse(job.last_run_at) : NaN;

      // Never fired is its own state, not a failure. A job added minutes ago
      // whose window has not come round yet is in exactly this position, and
      // painting it as a fault would be the crying-wolf indicator the branch
      // above was careful to avoid.
      if (Number.isFinite(ranAt)) {
        firedEl.dataset.beat = "beating";
        firedEl.textContent = "Last ran " + sinceWords(Date.now() - ranAt);
      } else if (!Number.isFinite(firedAt)) {
        firedEl.dataset.beat = "unknown";
        firedEl.textContent = "Never fired";
      } else {
        firedEl.dataset.beat = "beating";
        firedEl.textContent = "Last fired " + sinceWords(Date.now() - firedAt);
      }
      li.appendChild(firedEl);
    }
    ...
    // TKT-1020, his fourth ask alongside the ordering/scroll fix (TKT-1018):
    // a clear control on every log window. This one owns only its own job's
    // entry in runLogs - the Map is the client's whole accumulated buffer,
    // and clearing one card must never touch another's.
    const logClear = document.createElement("button");
    logClear.className = "jobs-card__log-clear";
    logClear.type = "button";
    logClear.textContent = "Clear";
    logClear.hidden = true;
    logClear.addEventListener("click", () => {
      runLogs.delete(job.id);
      paintLog(logBox, job.id);
    });
    // A MONITOR'S OWN WORDS, WHICH THIS PAGE NEVER ASKED FOR UNTIL NOW.
    //
    // runLogs holds what a Run-now click brought back, and that is all this
    // panel has ever shown. A monitor has no Run-now - its button is Start,
    // decided at TKT-843 CHK-001 because a monitor has no schedule to bypass -
    // and starting one returns a job record rather than output. So a monitor's
    // card could never show a line however much it was saying. His report,
    // 2026-09-04, beside a screenshot of two running monitors with empty
    // panels: "why only 1 job got the tail logs?". TKT-922.
    //
    // SEEDED, NOT PAINTED SEPARATELY, so there is one rendering path rather
    // than two: the record's tail goes into runLogs the way a run's output
    // does, and paintLog below is unchanged.
    //
    // TKT-1030. Re-synced on every poll where the server's own tail has
    // moved on, not seeded once and left - his report, with a screenshot:
    // the card said "Last ran 5 minutes ago" while its panel still showed
    // an old failure from an earlier run. The original "only when nothing
    // is remembered" guard fixed monitors showing nothing at all (TKT-922)
    // by trading it for monitors showing the SAME nothing-new for ever,
    // once seeded - a card rebuilt every poll never looked at job.recent
    // again after its first paint. seenRecent remembers the server's own
    // array as last observed per job, so a later poll can tell what is
    // genuinely new.
    //
    // OVERLAP, NOT A SINGLE LAST-LINE SENTINEL - Codex review caught the
    // first version of this matching only the previous tail's OWN last
    // line, which a repeated line anywhere in the new tail could satisfy
    // early and silently drop everything genuinely new after it. This
    // instead finds the largest k such that the OLD tail's last k lines
    // equal the NEW tail's first k lines - the shared portion a normal,
    // append-then-truncate tail always has - and takes only what follows
    // it as new. No overlap at all (the buffer rotated past everything
    // previously seen) falls back to the whole new tail; a little
    // duplication once in a great while beats silently freezing on
    // however old the client's own last look happened to be.
    if (Array.isArray(job.recent) && job.recent.length) {
      const already = seenRecent.get(job.id);
      if (!already) {
        rememberLines(job.id, job.recent.join("\n"));
      }
      else {
        const maxK = Math.min(already.length, job.recent.length);
        let overlap = 0;
        for (let k = maxK; k > 0; k--) {
          let matches = true;
          for (let i = 0; i < k; i++) {
            if (already[already.length - k + i] !== job.recent[i]) { matches = false; break; }
          }
          if (matches) { overlap = k; break; }
        }
        const fresh = job.recent.slice(overlap);
        if (fresh.length) rememberLines(job.id, fresh.join("\n"));
      }
      seenRecent.set(job.id, job.recent.slice());
    }
