...
close $env;
like( $env_text, qr/^VERSION=1\.39$/m, '.env stores version 1.39' );
...
is( scalar @use_cases, 133, 'SKILLS.md contains exactly 133 numbered use cases' );
my %seen;
while ( $skills_text =~ /^### UC-(\d{3}):/mg ) {
    $seen{$1}++;
}
is_deeply( [ sort keys %seen ], [ map { sprintf '%03d', $_ } 1 .. 133 ], 'use cases are numbered UC-001 through UC-133' );
...
use lib 'lib';
use Tira;
is( $Tira::VERSION, '1.39', 'module version matches .env' );
...
is( scalar @commands, 137, 'release ships exactly 137 executable CLI entrypoints' );
