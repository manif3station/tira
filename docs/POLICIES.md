...
| `answer-unjudged` | `--age`, `--read-age` | an answer nobody marked. The second age is optional and runs from when it was read. | **Since TKT-584 this is a backstop rather than the only prompt**: a card cannot move forward out of the column an answer was given in while that answer carries no mark, so the ask arrives at the moment the answer is being acted on rather than hours later. This rule still catches whatever escapes that - a card discarded, or a board where the move never comes - and is unchanged.
