...
if (jobsSection) {
  ...
  const jobRow = (job) => {
    ...
    //
    // ONLY WHEN NOTHING IS REMEMBERED. A card is rebuilt on every poll, and
    // re-seeding each time would push the same lines back in for ever and
    // reset their freshness, so a monitor that said one thing an hour ago
    // would look like it had just spoken - the exact wrong answer for a panel
    // whose colour means "recent".
    if (!runLogs.has(job.id) && Array.isArray(job.recent) && job.recent.length) {
