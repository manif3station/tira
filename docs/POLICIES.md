...
**What a command demands is in its own usage line.** Marking a required action or a checklist item done costs a `--command`/`--proof` pair, and until 4.55 `--help` did not say so: six of the commands an agent touches at every gate had no usage line at all and described themselves as taking `[options]`, while `tira.checklist.update` had one that listed three optional flags and omitted both mandatory ones - the worse shape, because it reads as exhaustive. Another project measured the cost: four `checklist.update` calls composed from `--help`, output suppressed, four entries believed ticked while the checklist still read 0/9, caught only by `checklist-unmoved`. The pair is now written as one bracketed unit, `[--command TEXT --proof TEXT ...]`, because it is required together and repeatable. Forty-nine commands still print a bare `[options]`; that set is recorded as a ledger a test holds, so it can shrink but not grow unnoticed. TKT-575.
