    ...
    if ( ref($data) eq 'HASH' && ref( $data->{questions} ) eq 'ARRAY' && exists $data->{instruction} ) {
        ...
        for my $question ( @{ $data->{questions} } ) {
            ...
            $body .= "## $question->{id} \x{2014} $question->{status}\n\n"
              . "$question->{text}\n\n"
              . "_Asked $question->{asked_at}$asked._\n\n";
