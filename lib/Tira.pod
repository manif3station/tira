...
B<C<evidence>/C<attachments>/C<gate_passing_log> refuse a non-array value,
since 5.161> (TKT-1130, self-caught live while pushing TKT-1028): C<evidence>
is a real CLI flag (C<--evidence>); C<attachments> and C<gate_passing_log>
have no CLI flag of their own but are recognised at the engine level in the
same shape. All three are a raw whole-array replacement kept for repair/
import, and C<attachments> doubles as C<record_clone>'s own passthrough
(TKT-609) - but the CLI always hands an option value through as a plain
string. C<record_update> used to write that string straight into the field
with no shape check, succeeding and printing the card back; every later
C<record_show> then died with C<Can't use string (...) as an ARRAY ref while
strict refs in use>. Now refused before the write - the outer value must be an ARRAY ref, and
(caught by a second review pass) every entry in it must itself be a HASH
ref, since a plain array of scalars corrupts the same way one level deeper
- naming the field and the one correct dedicated verb for it
(C<evidence_add>, C<attachment_add>, or C<gate_add>). A genuine ARRAY ref
of hash records still passes through unchanged.
...
Since 5.175 (TKT-1118), the read-modify-write of the suspension itself and
the C<_enforcement_record> call that logs it both run under
C<_with_enforcement_lock>, sharing one exclusive lock on the store rather than
either being unguarded. C<_with_enforcement_lock> is reentrant precisely
because C<rule_suspend> calls C<_enforcement_record> while already holding it
- a plain non-reentrant lock would have deadlocked on that second
acquisition, since C<flock()> blocks a second open file description on the
same path even from the process that already holds it.

A follow-up Codex pass, run while documenting this same ticket, found that
C<_announce_upgrade> (TKT-1114's own fix, see C<_announce_upgrade> below)
only took C<_with_project_lock> - a different mutex, keyed on the project
root rather than the store path - so it still shared no lock with any of
the five functions above even after they were fixed. It now takes
C<_with_enforcement_lock> too, nested inside its existing project lock,
so every C<_enforcement_read>/C<_enforcement_write> pair in this module
now shares one lock.
...
=head2 clearance_list

Cards whose questions are all answered since the last all-clear sent for
them - the "welcome back" half of a reminder, read by C<notification_message>.
Lives in L<Tira::Notification> since TKT-1102.
...
Returns the project's standing warnings. Lives in L<Tira::Notification>
since TKT-1102.

=head2 warning_add

Adds a warning, or returns the existing one if the same message is already
standing. Lives in L<Tira::Notification> since TKT-1102.
...
TKT-1143: when a cron schedule restricts both day-of-month and weekday
(neither is C<*>), POSIX cron matches on either field, not both - a schedule
of C<0 0 1 * 1> is due on the 1st of the month I<and also> every Monday, not
only on a Monday that happens to be the 1st. A schedule restricting at most
one of the two day fields is unaffected.
...
can never be stored as if it were a real link. TKT-824: also runs
C<tasklist_add>'s own C<possible_duplicate> soft-signal check (via the
shared C<_tasklist_possible_duplicate> helper) against every ref the item
carries once linked - the browser dashboard's add-then-link flow never
went through that check at all before this, so a person working entirely
through the browser got none of TKT-806's protection.
...
Records that a reminder was sent for one or more cards in a given column,
so the same reminder is not sent twice. Lives in L<Tira::Notification>
since TKT-1102.

=head2 notification_level

How many reminders a card has already had in its current column since the
last all-clear - the count C<notification_record> and
C<notification_message> both derive their own answers from. Lives in
L<Tira::Notification> since TKT-1102.

=head2 notification_list

Returns recorded notification sends, optionally filtered by ref. Lives in
L<Tira::Notification> since TKT-1102.
...
that no longer exists. TKT-613. The retag walk survives one bad record: each
is retagged inside its own try, and the returned hash carries a
C<retag_failed> array naming any ref whose own retag could not complete -
the directory and config rename have already committed by the time the
walk runs, so a record that cannot be read no longer aborts retagging
everything after it. A record whose embedded C<ref> disagrees with the ref
its own filename names is refused rather than written back under the
embedded ref, which would send the write looking for a different file
entirely. TKT-771.
...
Every column, including C<discard>, is always in the result - there is no
C<include_discard> to read here, deliberately: excluding discard is each
CALLER's decision (C<policy_evaluate> greps it out for itself when a rule
needs to; several callers want everything, including discard, and always
have). Teaching this method to filter would silently change what every
other caller gets. Seven callers passed C<include_discard =E<gt> 1> anyway
until TKT-970 removed it - the argument did nothing, since nothing here
ever read it, and it taught the opposite of the truth: that the default
hides discard and this flag opts back in.
...
Moves a card to another column, running the journal-attribution check along the way. Refuses a move with no author (TKT-457). Since 5.188 (TKT-1144) also refuses a forward departure with unmet, non-exempt required items by default - the check used to live only in the CLI dispatch layer's own C<_column_required_action_violation>, so any caller reaching this method a different way skipped it entirely. Exempted via the same C<--exempt-required>/C<--exempt-reason> mechanism as the CLI path; a backward move, a move to C<discard>, and a no-op move are all unaffected. Skipped for calls genuinely made from C<Tira::CLI::Browser>'s own source (checked via C<caller()>, never trusted from a caller-supplied argument) - a human moving a card on the HTML dashboard is not an agent skipping a gate (TKT-426). The column chain and unjudged-answer checks still live only in the CLI dispatch layer (TKT-1145 tracks lifting those too). Since 5.128 (TKT-785) the returned record also carries C<previous_column>, the column it moved the record out of - captured inside the same project lock that performs the actual rename, so it names the column THIS call moved the record from rather than a value read by a separate, unlocked call that could race a concurrent writer.
...
Since 5.146, a caller who passes C<< body => ... >> instead of C<< text => ... >>
(comment's own stored field, mistaken for the write flag) gets a refusal naming
C<--text> rather than the plain message - measured live in three fixtures in
this repository that made exactly that mistake and stored an empty comment for
as long as they existed. The message is derived from C<%WRITE_READ_PAIR>, the
table naming all four write/read pairs docs/commands.md describes, so a pair
that starts disagreeing later has one place to update rather than a second
message to write. A genuinely empty C<--text> still gets the plain refusal -
the two cases stay distinguishable. TKT-908.
...
done. Since 5.139 (TKT-846) the declared set also accepts C<todo> (no
space) - the legacy spelling this board itself had already stored on
several cards before the validation existed, which the stricter set then
refused to write back unchanged. Since 5.62 (TKT-909) C<--item> refuses a whitespace-only value the
...
Since 5.147, a forbidden option's refusal names what THAT option actually
means rather than one sentence written for C<--age>. C<card-unassigned>
forbids C<column>, C<enter> AND C<age>; declaring it with C<--column> used to
get back "a grace would only delay it" - true of C<--age>, and not what
C<--column> is at all. The reason is now looked up in C<%FORBID_REASON>,
keyed by option name rather than stored per-rule, since every rule forbidding
the same option wants the same reason today. C<--age>'s own wording is
unchanged. Codex review found two gaps in the first draft: C<unpushed-work>
forbids C<--pattern> with no entry in the table, which silently fell back to
the C<--age> wording (false - pattern is refused because the rule's own body
never reads it); and C<column-unwatched> forbids C<--column> because it is
about which columns OTHER policies name, a whole-board reason, not the
card-scoped one every other rule forbidding C<--column> wants - fixed with
C<%FORBID_REASON_FOR_RULE>, checked before the option-only table. TKT-967.
...
Writes a board's shape - per-type columns (the column chain lives inside
each column's own C<next>), prefix, digits, and the declared/declined
policy ledgers - as JSON to C<--file FILE>. Read-only; never writes to the
board it reads from. Cards, jobs and tasks are deliberately out of scope.

=head2 schema_import

The other half of TKT-1123, called by C<tira.onboard --from-schema FILE>
right after C<project_new> lays down a fresh project's skeleton. Overwrites
each type's columns/prefix/digits and the project's policy ledgers (with
C<policy_counter>) with what a prior C<schema_export> captured;
C<next_number> is left alone, so the new project still starts counting at
1. Refuses per type if the target already has real records, and validates
the schema's own shape (columns, prefix, digits) before writing anything -
the whole check-then-write runs inside one held project lock.
...
=head2 police_explain

TKT-786: prints why one police rule reaches its verdict on one card, reading
the SAME inputs the rule itself reads rather than a second copy of the
logic that could drift from it. Covers C<discard-unexplained> today (calls
C<_discard_unexplained_inputs>, the identical helper the real rule body
calls); C<card-duration>, C<agent-still> and C<board-still> are refused by
name, tracked as TKT-1106.

=head2 police_freshness

Returns C<< { taken_at, age_seconds, unreadable, stale } >> for a police store:
when the last pass ran, how long ago that was, and whether the answer can
still be trusted.
...
C<unreadable> (TKT-1094) is C<undef> when C<age_seconds> parsed cleanly,
C<'pass'> when the stored C<last_pass> stamp would not parse - the only case
real production ever reaches - or C<'clock'> when the live clock reading used
to compute the age is what failed instead, which needs an injected clock (a
test) to reach at all. It exists so a caller reporting the failure (the CLI's
UNREADABLE message) can name the side that actually broke, rather than always
...
Two independent kinds of damage, found by two independent passes over the
same files. The first, since 1.94, is a byte-level check: any byte that is
not valid UTF-8, repaired (with C<--repair>) by reading it as latin-1 and
writing it back as UTF-8. The second, since 5.162 (TKT-1131, found by an
improvement hunt while working TKT-1130's C<record_update> fix), is a
structural check: a card record whose C<evidence>/C<attachments>/
C<gate_passing_log> is C<null> or not an ARRAY ref of HASH refs - the shape
C<record_update> now refuses to write, but a record corrupted that way
before 5.161 shipped, or written directly rather than through the engine,
is perfectly valid JSON and so invisible to the byte check. C<null> counts
as damage too, not a legitimate "no value": C<record_update>'s own guard
treats undef as "never asked" at write time, but a record already holding
one crashes C<evidence_add>'s C<@{$record-E<gt>{evidence}} + 1> the same
way a plain string does. C<--repair> resets an offending field to C<[]>;
there is no way to recover the intended array content from a corrupted
scalar, and doing so rewrites the whole record through the canonical JSON
encoder, so its formatting/key order can change even though content
otherwise does not - unlike the byte repair, which touches only the bad
byte. Both kinds of finding share the same C<damaged>/C<repaired> result
lists.
...
compared case-insensitively against the values C<checklist_add> accepts -
C<pending>, C<done>, C<To Do> and (since 5.139, TKT-846) C<todo>. Anything outside that set is refused, naming
