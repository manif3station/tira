...
| `--stop` | no | Since 5.159 (TKT-1125): stop an already-running board for this project - the same signal Ctrl-C on its own terminal already sends, found by port rather than a pidfile, with any `--with-police`/`--with-policy-bridge` companion killed directly by its own recorded pid. Refused with a reason (`not-running`/`no-board`/`not-a-board`/`still-running`) if there is nothing to stop, something on the remembered port that is not a Starman, or the signalled master is still answering after the whole wait. Belongs only to `tira.dashboard`/`.sow`/`.epic`/`.ticket` - refused on every other command. |
| `--restart` | no | Since 5.159 (TKT-1125): the same stop, then re-execs the exact script and argv the board was originally started with - `--with-police`/`--with-policy-bridge`/everything else it was launched with, remembered rather than retyped. Mutually exclusive with `--stop`. |
...
**Since 5.188 (TKT-1144), check 2 also runs inside the engine's own `record_move`, not only in the CLI dispatch layer.** The chain check and the unjudged-answer check still live only in `Tira::CLI.pm`'s dispatch (tracked separately, TKT-1145) - but the required-action departure check used to be reachable only through the ordinary `d2 tira.<type>.move` path, so any caller reaching `record_move` a different way (a plugin, a script, direct-Perl automation) skipped it entirely, with no exemption and no reason recorded. Confirmed live: an automation identity moved a real card through 5+ columns this way, leaving each column's own required items pending and unexempted throughout. `record_move` now refuses that departure by default, with the identical message the CLI's own pre-check already gives - the two can never disagree about which moves would have been allowed, since the CLI's own check still runs first on the ordinary path. `--exempt-required`/`--exempt-reason` remains the only sanctioned way through, on either path. The HTML dashboard's own move route (TKT-426: a human clicking a card is not an agent skipping a gate) is unaffected - `record_move` checks, via `caller()`, that it is genuinely being invoked from `Tira::CLI::Browser`'s own source, not from anything a caller-supplied argument could claim.
...
**`--brief`, since 5.87** (TKT-669): each item comes back as only its id, column, status and text, proof dropped - a card with several multi-paragraph gate proofs was unreadable when the actual question was which ids remained pending. Previously refused with "Read options are available on show, list, and export commands", a message that read as though this command should already answer to it; it is now named directly.
...
**One column, more than one threshold, since 5.89** (TKT-756): `card-duration
...
- **`tira.attachment.where --sha SHA256 [--extension EXT] [-o FORMAT]`, since
  5.89 (TKT-766).** `attachment_list` only ever answers "what does THIS
...
- `tira.TYPE.move --ref REF --column SLUG [--author NAME] [-o FORMAT]` — a successful move stamps a `moved` field onto the record it returns, `"REF moved: FROM -> TO"`, since 5.128 (TKT-785): `-o human` prints it as the first line, ahead of the unabridged full record that still follows; `-o json`/`-o toon` carry it as an ordinary field alongside every other one, never in place of them. Before this, a successful move returned the same full record dump `tira.TYPE.show` gives, with the one fact that actually confirms it worked - the new column - sorted wherever it happens to land among comments, required_items, checklist, gate_passing_log and evidence. Measured cost: a real card sat stalled in `verify` for roughly two hours because its own move to `pending-push` was never confirmed to have landed, caught only by an unrelated 30-minute police poll. A refused move is unaffected - it dies before reaching this point, so it never claims to have moved anything. A no-op move onto the column a card already rests in is also unaffected - `moved` is only set when the destination genuinely differs from where the record was, so it never reads `"REF moved: X -> X"` for a call that changed nothing. The field also always names the record's own current `column` as the destination, rather than repeating the caller's literal `--column` value, so it can never disagree with the full record it sits beside - two gaps Codex review found: a caller could report a no-op as a move, and a second, unlocked re-read taken after `record_move`'s own lock releases (to pick up required-action/tasklist side effects) could in principle race a concurrent writer and show a `column` that no longer matched the literal destination named in `moved`.
...
- `tira.TYPE.discard --ref REF [-o FORMAT]` — takes no reason; a discarded card is explained with `tira.comment.add`, which is what `discard-unexplained` looks for. A successful discard stamps the same `moved` field `tira.TYPE.move` does, `"REF discarded: FROM -> discard"`, since 5.172 (TKT-1096) - `record_discard` is a thin `record_move` wrapper and already carried `previous_column`, but the CLI dispatch called it directly and never reached `move`'s own confirmation-stamping, so a successful discard printed the same unabridged full record dump `tira.TYPE.show` gives, with no confirmation at all.
- `tira.TYPE.restore --ref REF [--column SLUG] [-o FORMAT]` — a successful restore gets the identical treatment, `"REF restored: discard -> TO"` (TKT-1096), the same gap and the same fix as discard above.
...
- `tira.checklist.add --ref REF --item TEXT [--status TEXT] [--command TEXT ...] [--proof TEXT ...] [--author NAME] [-o FORMAT]` - **`--author`, since 5.89 (TKT-712), is documented at last** - `checklist_add` has always refused without one (or `TIRA_AUTHOR`), and this usage line named every other argument, reading as complete; a call composed from it alone was refused with no hint which argument was missing. `checklist.update` and `tira.TYPE.update` had the identical gap and get the identical fix. **This document's own line above was right all along; SKILLS.md's separate usage catalogue was not** - it kept the old `[record field arguments]` form with no `--author` at all until TKT-1019 (5.153) caught the drift between the two documents. Since 5.62 `--item` refuses a whitespace-only value, naming what is missing, the same as the literal empty string always did - the same fix `evidence.add` and `required-action.add` got, and the same reasoning TKT-585 already settled for `--command`/`--proof`: whitespace counts as empty. `checklist.update` is deliberately unchanged - blanking an existing item is a different question from creating a blank one, matching TKT-753's precedent for `comment.update`. TKT-909. **Since 5.85, `--status done` also needs a `--command`/`--proof` pair** (TKT-958) - the same requirement `checklist.update` already enforced on every later write, which creating an item already done used to bypass entirely. **`--status` defaults to `To Do` when omitted or given as an empty string** (TKT-574) - it used to be required with no default, so every `checklist.add` typed without it failed once before being retried with `--status "To Do"`; measured across three cards filed in one evening, five of fifteen follow-up `checklist.add` calls failed on exactly this before succeeding on the retry. `To Do` is the only sensible default: an item created already `Done` has nothing left to prove and the checklist gates would have nothing to mark. An explicit `--status` still wins, unchanged. **Since 5.111 (TKT-642), the next `CHK-NNN` id is the highest existing number plus one, not a count of the list** - so an entry removed by hand (there is no removal command) never collides with an id still on the list, matching `conversation_add`'s own scheme. It does not protect the one id that was itself the highest: removing that entry and adding another reissues the exact number just freed - the same known limit `conversation_add` already had; avoiding that case too needs a persisted counter, the way task ids get one, deliberately out of scope here.
- `tira.checklist.list --ref REF [--status STATUS] [-o FORMAT]`
  - `--status STATUS` (TKT-748) narrows to items in that status, taking the
    same values `checklist.add` accepts — `pending`, `done`, `To Do`, and
    (since 5.139, TKT-846) `todo`, compared case-insensitively. `To Do` is
    included because it is the spelling this board itself writes on move-in;
    `todo` (no space) is included because several cards on this board had
    already stored it before the validation existed, and a filter that
    understood only the other three would make those items unfindable too.
  - A value outside those four is **refused**, naming what was given, rather
  ...
  - The filter is in the engine rather than the option parser, so the browser
    ...
    TKT-713.
- `tira.checklist.update --ref REF --id CHK-NNN [--item TEXT] [--status TEXT] [--column SLUG] [--command TEXT ... | --command-file PATH ...] [--proof TEXT ... | --proof-file PATH ...] [--author NAME] [-o FORMAT]` - `--column`, since 5.89 (TKT-700), is checked against the CARD's own current column when given, refusing a disagreement rather than silently accepting it - see `required-action.update` below for the full reasoning and the incident that found it; a checklist entry has no column of its own, so this checks against the card's rather than the entry's. **`--command-file`, since 5.89 (TKT-711), gives `--command` the identical file/stdin path `--proof-file` already had** (TKT-674): an agent's real captured command line is often long or full of quoting hazards, and the only way in otherwise was pasting a summary - the behaviour the evidence rule exists to prevent. A single dash reads stdin. Supplying both a literal `--command` and a `--command-file` for what would be the same pair is refused the same way mixing `--proof`/`--proof-file` for one pair already is. Marking `--status done` (case-insensitively) refuses without at least one `--command`/`--proof` pair, repeatable for an item that took several commands to satisfy, and refuses when either half of a pair is empty or whitespace-only - naming the half, rather than repeating the message for a missing pair, since a caller who supplied one and is told to supply one has been sent back to what they already did. `--proof` is the literal output of the paired `--command`, trusted as given: emptiness is checked, quality is not. Every other status change is unaffected. Caught on ZSD-246: a checklist backfilled after the fact, marked Done the instant it was typed - proof is what a done claim now costs. TKT-453. An unknown `--id` refuses naming the card's real ids (or the `CHK-NNN` shape, on a card with none yet) rather than only "not found" - an ordinal like `--id 1` used to fail exactly that way with no hint that ids are not positions, and a caller looping ordinals over a checklist found every call silently failing. TKT-280. **Since 5.118 (TKT-693), `--id` also accepts an entry's own exact text** - the wording an agent actually has in front of it, printed on the card, rather than an id it would otherwise have to run `ticket.show` to find. An unambiguous exact match on `item` is treated exactly as its id would be; text matching more than one entry refuses, naming every matching id, and changes nothing; text matching no entry falls through to the same not-found refusal above. `--item` is unaffected and still renames the entry addressed by `--id` - it does not become a second way to address one.
...
- `tira.required-action.update --ref REF (--id REQ-NNN | --ids REQ-NNN,REQ-NNN,... [--ids ...]) [--item TEXT] [--status TEXT] [--column SLUG] [--command TEXT ... | --command-file PATH ...] [--proof TEXT ... | --proof-file PATH ...] [--repeated-reason TEXT] [--repeated-confirm CODE] [--author NAME] [-o FORMAT]` - **`--author`, since 5.153 (TKT-1019), is documented at last** - `required_item_update` has always refused without one (or `TIRA_AUTHOR`), the same gap TKT-712 fixed for `checklist.add`/`checklist.update`/`required-action.add`/`comment.update`/`tira.TYPE.update`, missed here because Codex review caught it on a later pass rather than the original audit. - **`--command-file`, since 5.89 (TKT-711), reads `--command` from a file or, via a single dash, stdin** - the identical path `--proof-file` already had (TKT-674), for the identical reason: a real captured command line is often long or hazardous to quote, and the only way in otherwise was a pasted summary. `checklist.update` gets the identical option, sharing this command's own pair-validation engine code; supplying both a literal `--command` and a `--command-file` for one pair is refused. **`--column`, since 5.89, is a guard rather than a selector** (TKT-700): the id already finds the item uniquely, so a `--column` that agrees with the item's own is unaffected, one that disagrees is refused naming both, and a call with no `--column` behaves exactly as before - it has not become required, because most calls are correct and the flag is already accepted by every command. Measured live on TKT-657: six calls named `--column next-to-work-on` and typed an id that, on that particular card's own numbering, belonged to a backlog item two ids off - the id was found, the column disagreed, and nothing checked. A successful call already names the item it marked, id and text, in its reply - unaffected by this fix, since the return has always been the whole item. `checklist.update` gets the identical guard, against the CARD's own current column rather than the entry's, since a checklist entry carries no column of its own. Same `--command`/`--proof` requirement on `--status done` as checklist.update above, and the same reasoning: a required item, gating or not, is not evidence of what happened just because it says so. TKT-453. An unknown `--id` refuses naming the card's real ids (or the `REQ-NNN` shape, on a card with none yet), the same fix checklist.update got for the identical bug. TKT-488. **Since 5.118 (TKT-693), `--id` also accepts an entry's own exact text**, the identical fallback `checklist.update` got at the same time and for the same reason: an unambiguous exact match on `item` addresses the entry, more than one match refuses naming every matching id and changes nothing, and no match falls through to the not-found refusal above. `--item` still only renames. **Since TKT-692, an absent `--id` refuses as absent, not as unknown** - `--item`/`--status` alone used to reach the not-found lookup with an undef id, leaking a warning and reporting that empty-string "id" was not found; the refusal now names `--id` directly and points out that `--item` renames an existing entry rather than addressing one, since that is what a caller who omitted `--id` most likely typed instead. `checklist.update` got the identical fix at the same time. Since 4.64 `--command` is usable without `--proof`: it records what is being run and the dashboard shows a clock. It travels with a `--status` or an `--item`, because the command still refuses a call that changes nothing - `--status pending --command "prove -lr t"` on an item already pending is the ordinary shape, and it leaves the status where it was. The proof arrives later, repeating its command. What marks the item done is still `--status done`, which refuses without the pair; supplying a full pair with `--status pending` is accepted, stored, and leaves the item pending - recording evidence and asserting a status are two different acts. So the pair rule is unchanged for done-claims and simply no longer required to say work has started. Until then the same call was accepted and discarded silently. The usage line reflects it: `[--command TEXT ... [--proof TEXT ...]]` - proofs nest inside commands as a group, because a proof cannot arrive without a command and, once any proof is given, the counts must match one for one. Announcing several commands at once is fine; announcing two and proving one is refused with `Every --command needs a matching --proof`. `tira.checklist.update` gained the same engine behaviour - the discard was in a function both share - but no clock, which is a required-action rendering. An announcement writes no `gate_passing_log` entry: that log records what was proved. Announcing over an item that is already done is refused unless `--status` re-opens it - otherwise renaming a finished item while announcing would leave it claiming done with an announcement where its evidence used to be. Two items may announce the same command; the reused-proof refusal is about evidence, and an announcement is not evidence yet. TKT-628. `--status` on `required-action.update` is checked, case-insensitively, against the declared set `{pending, done}` (TKT-668, Q-099) - a misspelling like `Donee` or `donw` used to exit 0 and store cleanly while no gate would ever recognize it as done, so it is now refused, naming the value given and the ones that work. `required-action.add` is deliberately NOT validated the same way: every real path that creates a required item (column entry/exit templates, the move-in population, a person backfilling one by hand) writes `pending` itself, so the misspelling this fix targets only ever reached the board through `.update`. `checklist.add`/`.update` both validate, against `{pending, done, To Do, todo}` (the last since 5.139, TKT-846) - `To Do` because it is the actual unmarked spelling this board itself writes on move-in, even though a required item never carries it, and `todo` (no space) because several cards on this board had already stored it before the validation existed. The refusal is at the point of writing only: a record whose status was written before this validation shipped still reads and moves normally. Within the declared set, only the comparison against `done` is otherwise defined - so `--status Done` marks an item finished for every gate, and since 4.63 for the browser dashboard too, which had compared against the literal `done` in the count, the icon and the checkbox and so showed items the gate considered finished as outstanding. `pending`/`To Do` are both genuinely unfinished, everywhere. Nothing rewrites what you store. TKT-434, TKT-601, TKT-668.
  ...
  **`--proof-file PATH` reads a proof from a file instead of a shell argument**, pairing positionally with `--command` exactly as a literal `--proof` does - `--proof` and `--proof-file` push onto one ordered list as `Getopt::Long` parses them, so mixing the two across several pairs in one call keeps each command with the proof it was given, whichever form supplied it. `--proof-file -` reads stdin, reusing `_text_input` the way `comment.add --file` already does. Real captured output often contains quotes, backslashes and newlines - exactly the content shell quoting mangles - so this reaches the card unchanged rather than needing to survive a shell argument first. Supplying both `--proof` and `--proof-file` for what would be the same single pair is refused, naming both flags, the same shape as `comment.add`'s `--text`/`--file` guard. `checklist.update` takes the same flag for the same reason. TKT-674.
...
**`--ids REQ-NNN,REQ-NNN,...`, since 5.89 (TKT-769), marks several ids with one `--command`/`--proof` pair.** A gated column's entry actions are typically several near-identical items whose honest evidence is the same command's output - already sanctioned once, via `--repeated-reason` above (TKT-585), but the mechanism still cost one full invocation per id. `--ids` accepts a comma-separated list, is repeatable, and is additive - `--id` is unaffected and stays the single, shared option every other command (`person`, `question`, `checklist`) also reads. Every named id must exist and be markable before any of them are marked - a bad id anywhere refuses the whole call, nothing is written, the same all-or-nothing principle TKT-485 established for creation. Ids named together in one `--ids` call are a declared batch and do not trip the reused-proof check against each other; `--repeated-reason` (with its usual two-step confirmation code) is still required only when the reuse is against an item outside the batch. `--ids` combined with `--item` is refused outright, since renaming several entries to the same text in one call is a materially different, riskier operation than marking several done with one proof.
...
- `tira.column.apply --type TYPE --columns-json JSON [-o FORMAT]` - a whole-layout replace, what the dashboard's Columns dialog Save button calls. Each entry may carry `next`, `required_actions`, `entry_required_actions`, and `administrative_actions`, same shape as `column.update --next`/`--required-action`/`--administrative-action`; until 3.14 a round-trip through this command silently dropped the first two even though the same layout read (`column.list`, and the dialog's own GET) already returned them - a saved layout that changed nothing about a column's chain or template would still lose them. `administrative_actions` (TKT-678) got the identical fix at the moment it was added, rather than repeating the gap for a third list. TKT-454. **Since 5.65 the whole call is one transaction.** Until then each removal took the project lock on its own and the rest of the layout - labels, order, `notify_after`, watched, new columns - was a separate transaction after it; a removal that failed partway through several left some columns gone (their cards already discarded) and the rest of the intended layout never attempted - neither the old layout nor the new one. Every removal and the final config write now run under one lock, and a failure at any point rolls back every card already moved back to the column it was resting in. TKT-767. **Since 5.127 a layout entry may carry `rename_from`** (TKT-772): a real rename, named explicitly by whatever control sets it, never guessed from the layout's own shape. The browser Columns dialog had no rename control at all - only `column.rename` on the CLI could rename a column without discarding its cards - so a caller renaming a column through the dialog's Save button used to remove the old name and add the new one, discarding every card the old column held. An earlier fix tried to detect a rename by shape (exactly one column removed, exactly one added, at the same position) and refuse it; Codex review proved that unsound, since `t/52-column-apply.t`'s own long-standing "Adding and removing in the same call" case removes one column and adds an unrelated one at the identical position - structurally indistinguishable from a real rename. `rename_from` replaces that guess with an explicit signal: an entry naming `rename_from => 'old-name'` delegates to `column_rename` itself, so the card-preserving, required-items-retagging behaviour a standalone `tira.column.rename` call already gives applies here too, whichever door a caller comes through. `rename_from` naming a column that is not currently on the board, or claimed by more than one entry at once, is refused by name before anything is written; renaming a protected column (`backlog`, `discard`) is refused the same way `column.rename` itself already refuses it. A layout entry with no `rename_from` is unaffected - a genuine remove-and-add, even one sharing a removed column's exact position, is still never mistaken for a rename.
...
- `tira.comment.add --ref REF --author ID (--text TEXT|--file FILE) [--format markdown|text] [--attach PATH ...] [--key-detail TEXT ...] [-o FORMAT]` - the stored/read field is `body`, the only one of four write/read pairs (gate `--details`/`details`, evidence `--summary`/`summary`, checklist `--item`/`item`) where the write flag and the field name disagree. Since 3.81 the returned comment also carries `text` alongside `body`, the same value, so a caller reaching for the write-side name on the way back out finds it rather than a silent `None` - built fresh on every read, never persisted onto the stored comment. TKT-353. Since 5.43 an empty or whitespace-only `--text` is **refused** — *A comment needs some text* — which `comment.add` was alone in accepting: every sibling refuses an empty value — `evidence.add` a summary, `warning.add` a message, `question.answer` some text, `checklist.add`/`required-action.add` an item. On **whitespace**, `warning.add`, `question.answer`, `evidence.add`, `checklist.add` and `required-action.add` all test `/\S/` too - `gate.add`'s `--details` was the last of this family still testing `eq ''` (measured live, correcting an earlier version of this line: `evidence.add` and its siblings already tested content, not presence, before this), fixed the same way in 5.89 (TKT-768). The test is `/\S/` rather than a length check, because a space is not a smaller comment than none (the reasoning TKT-585 settled for `--command`/`--proof`) and because it is the **same test** `discard-unexplained` applies to a comment body when deciding whether a card was explained — so the command and the rule cannot drift apart about what counts as saying something. It lives in the engine, so the browser dashboard's comment provider is guarded by the same rule rather than by a second copy of it. TKT-753. Since 5.146, a caller who passes `body => ...` (comment's own stored field, mistaken for the write flag) gets *A comment needs some text - use --text, not body* instead of the plain refusal - measured live in three fixtures in this repository that made exactly that mistake and stored an empty comment for as long as they existed. The message is derived from `%WRITE_READ_PAIR`, the same table naming all four write/read pairs, so it does not drift into a second copy if another pair ever starts disagreeing. A genuinely empty `--text` still gets the plain refusal - the two cases stay distinguishable, since a caller who supplied nothing at all was never told they passed the wrong flag. TKT-908. **`--key-detail TEXT` (repeatable), since 5.171** (TKT-1090): folds the comment into the card's durable `key_details` field in the same call, rather than needing a separate `tira.<type>.update --key-detail` afterward to satisfy `conversation-not-folded`. Checked before either half is written: a whitespace-only key-detail refuses the whole call, the same all-or-nothing shape `--text` already has - not the comment written and the key-detail silently dropped. Omitting it leaves `comment.add` exactly as it was.
...
- `tira.comment.update --ref REF --comment ID (--text TEXT|--file FILE) [--format markdown|text] [--author NAME] [-o FORMAT]` - the updated comment also carries `text` alongside `body`, same as `comment.add`. An unknown `--comment` id refuses the same way as `comment.remove`. TKT-491. **Since TKT-692, a missing `--comment` refuses as missing, not as unknown** - it used to fall into the not-found lookup with an undef id, leaking a warning and reporting that empty-string "id" was not found rather than that none was given.
...
- `tira.evidence.annotate --ref REF --id EVD-NNN --note TEXT [--author ID] [-o FORMAT]` - an unknown `--id` refuses naming the card's real evidence ids, or the `EVD-NNN` shape on a card with none yet, the same fix `checklist.update` (TKT-280) and `required-action.update` (TKT-488) already got. TKT-490. An absent `--id` refuses as absent, the same TKT-692 fix `required-action.update` got, shared through the annotate helper both `gate.annotate` and `evidence.annotate` call.
...
- `tira.tasklist.list [--session ID] [--all-sessions] [--status STATUS] [--unlinked] [--ref REF] [--sort FIELD:DIR[,FIELD:DIR...]] [-o FORMAT]` — **since 5.102, `--sort` and `--unlinked` are refused on every other command, and `--all-sessions` on every command but `tira.search --tasklist`** (TKT-581): `--sort`/`--unlinked` are read in exactly one place in the whole engine, `Tira::Tasklist::tasklist_list`; `--all-sessions` in two, the other being `search`'s own TKT-550 cross-session tasklist match. Before this a command like `tira.ticket.show --sort text:asc` exited 0, printed the card, and silently ignored the flag - a caller who believed they had asked for an ordering or a filter got one that was never applied, with nothing said. The refusal names the flag and points at the command(s) that read it. `--sort` takes `asc` or `desc`, and **`DESC` in any capitalisation is read as `desc`** because that is how SQL and every spreadsheet write it. **Anything else is refused**, naming what was given and what is accepted: since 5.42 a direction the parser cannot honour, and a field a tasklist item does not have, are both errors rather than silently wrong answers. Before that, `status:DESC` returned *ascending* order and `bogus:desc` returned the list unsorted — a wrong answer that looked exactly like a right one (TKT-888). The sortable fields are `created_at`, `id`, `last_updated`, `order`, `session`, `status` and `text`. The default is `last_updated:desc,status:asc`, and a bare field with no direction means ascending. **Since 5.90 each item also carries `status_name`** (TKT-853) — `pending`, `working` or `done` — alongside the stored `status` code (0/1/2), which stays exactly that: `tasklist.add`/`tasklist.update` still return and accept the code, so a machine reader keeps the same stable key it always had, and a human reading several items no longer has to open `lib/Tira/Tasklist.pm` to learn what `1` means.
...
- `tira.tasklist.task.ref.link --id ID --ref REF [--ref REF ...] [--session ID] [-o FORMAT]` — **the `[--ref REF ...]` shown here actually works, since 5.89** (TKT-791): a generic batch-ref guard refused any command outside the internal `record.show` dispatch value (reached by the three show verbs), `tasklist.next`, and `release.record` the moment a second `--ref` was given, so two calls documented here were refused before this command's own dispatch - which already read every `--ref` correctly - was ever reached. `notify.record` had the identical gap and the identical fix. **Since 5.137, also carries `tasklist_add`'s own `possible_duplicate` soft signal** (TKT-806, extended by TKT-824): linking a ref that another pending/working item already carries surfaces the identical `{ id, text }` warning `tasklist_add` gives at creation time, via the same shared helper both now call - a person working entirely through the browser dashboard's separate add-then-link flow used to get none of that protection.
...
- `tira.job.help`
    ...
    **And since 5.52 a hidden row sends null too**, which is not the same
    ...
    (TKT-917). **Nothing renders as raw cron**: anything it cannot say exactly is
    marked and says why — `About every 7 minutes (restarts each hour)` — and the
    day-field OR is stated outright rather than hedged, since `0 0 1 * 1` fires
    on the 1st *and* every Monday and that is exact rather than approximate. And
