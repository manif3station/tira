...
sub _tasklist_store_attachment {
    ...
    my $content = do { local $/; <$fh> };
    close $fh;
    return $self->_tasklist_store_attachment_bytes( $root, basename($path), $content );
}
...
sub column_update {
    ...
    return $self->_with_project_lock( $root, sub {
        ...
        # different questions of a card and a board declares them separately,
        # which is also why the exit list is untouched by an entry
        # declaration. The owner's example is the shape of it - "between
        # backlog to red-test - Ther is a item needs to be done before
        # red-test but not belongs to backlog - Verify all details in the
...
sub checklist_update {
    ...
    return $self->_with_project_lock( $root, sub {
        ...
        $entry->{last_updated} = $self->{clock}->();
        # An announcement must not overwrite evidence behind a done claim.
        # $proof_entries became truthy for announcements in TKT-628, and this
        # assignment is unconditional - so renaming a done item while
        # announcing a command replaced its proof with a command-only entry,
        # leaving the item still saying done with nothing behind it. That is
        # the state TKT-583 and TKT-585 exist to prevent, reached from a new
        # direction. An announcement is only stored when the item is not
        # standing on a proof it still needs.
        my $announcing = $proof_entries
          && !grep { defined $_->{proof} || $_->{attachment} } @{$proof_entries};
        my $resulting = lc( ( defined $args{status} ? $args{status} : $entry->{status} ) // '' );
        die "That item is done and its proof is what says so - re-open it with "
          . "--status before announcing a new command over it.\n"
          if $announcing
          && $resulting eq 'done'
          && grep { defined $_->{proof} || $_->{attachment} } @{ $entry->{proof} // [] };

        $entry->{proof} = $proof_entries if $proof_entries;

        # The gate log records a DONE claim, and its entries read
        # 'Marked "..." done'. Keyed off the RESULTING status rather than
        # what the caller passed: a done item given fresh evidence without
        # repeating --status stored it and logged nothing, because $args{status}
        # was absent. What is recorded and what the log records cannot diverge.
        $self->_log_proof_gate( $record, 'checklist', $entry, $proof_entries )
          if $proof_entries && !$announcing && $resulting eq 'done';
...
# One piece of evidence cannot prove two different instructions. TKT-453 made
# a done claim cost a --command/--proof pair; it did not stop the same pair
# being spent on every item in a column. The owner found it on his own board:
# "the agent will use the same command and message to fill in and mark all
# done. Like TKT-555 Install column. All required action items ran command
# and proof all the same. And that is one of many." TKT-555 had six items
# sharing one pair; TKT-570 had eleven - a single prove run answering
# instructions like "add the start date", which it plainly did not.
#
# Scoped to the column, which is where the instructions differ from one
# another and where the pattern appears. The same pair in a DIFFERENT column
# is left alone: those are a different set of instructions and a command can
# honestly bear on both.
#
# Compared after trimming, so a trailing newline cannot slip identical
...
sub _refuse_reused_proof {
    ...
    return if !$proof_entries;

    # An announcement is not evidence, so it cannot be evidence used twice.
    # This rule is about passing one PROOF off as proving two things; a
    # command with no proof yet proves nothing about anything. Without this,
    # two items announcing "prove -lr t" in one column looked identical -
    # _tidy signatures a missing proof as the empty string, so the second was
    # refused for reusing a proof that did not exist. TKT-628 created that;
    # announcing the same command on several items is ordinary.
    my @proved = grep { defined $_->{proof} || $_->{attachment} } @{$proof_entries};
    return if !@proved;
    $proof_entries = \@proved;

    # The door is priced rather than locked. The owner's words: "if the agent
    # thinks using the same command and proof set for other required action
    # item ... They need to provide a valid reason for that." One suite run
    # can honestly prove two items - "the suite passes" and "no regression" -
    # and refusing that outright would make the check something to work
    # around rather than something to satisfy.
    #
    # The reason must have content. An empty one would rebuild the hole
    # TKT-585 records, where a gate counts an argument instead of reading it.
    ...
    my $signature = sub {
        my ($pair) = @_;
        my $proof = exists $pair->{attachment}
          ? 'sha:' . ( $pair->{attachment}{sha} // '' )
          : $tidy->( $pair->{proof} );
        return $tidy->( $pair->{command} ) . "\0" . $proof;
    };

    my %wanted = map { $signature->($_) => 1 } @{$proof_entries};
    my $column = $entry->{column} // '';
    for my $other ( @{ $items // [] } ) {
        next if ( $other->{id} // '' ) eq ( $entry->{id} // '' );
        ...
        for my $pair ( @{ $other->{proof} // [] } ) {
            ...
            die "That --command and --proof already prove $other->{id} in this column"
              ...
              . " Run what THIS item asks for and record what that produced.\n"
              . "If the same run genuinely proves both, say why with"
              . " --repeated-reason, and the reason is kept on the item.\n";
        }
    }
    return;
}
...
sub _proof_entries_for {
    ...
    my $done = defined $args{status} && lc( $args{status} ) eq 'done';

    # An ordinary status change carrying no evidence at all - the overwhelming
    # majority of calls - records nothing and is not asked to. This is the only
    # remaining early return, and it is about having nothing to store rather
    # than about which status was named.
    return undef if !$done && !@commands && !@proofs;

    # Until 4.64 this returned undef for ANY status that was not done, so a
    # --command given with --status pending was parsed by the CLI, accepted,
    # and dropped here before the pair logic was ever reached - the call
    # succeeded, nothing was stored, and the caller was told nothing. The owner
    # asked for the opposite (TSK-194): "Agent can provide the command for a
    # specific item first, so the user can see it is working on it". The item
    # stays pending, the command is kept, and the proof arrives afterwards.
    # Same shape as TKT-625's ignored --dry-run: a flag accepted and discarded.
    #
    # Both families reach this, and both stop discarding. The early return was
    # never scoped to required actions, so leaving checklist items dropping a
    # lone command would have meant ADDING a condition to preserve the bug for
    # one of them. Q-092: "Fix the engine for both, render the clock on
    # required actions only." The clock is the dashboard's business; this is
    # about what is kept. TKT-628.
    die "Marking done requires at least one --command/--proof pair\n"
      if $done && ( !@commands || !@proofs );

    # A proof with no command is a claim about a command nobody named, whatever
    # the status - so it is refused on the way in rather than stored half-made.
    die "A --proof needs the --command it came from\n" if @proofs && !@commands;

    die "Every --command needs a matching --proof, and every --proof a --command "
      . "(got " . scalar(@commands) . " command(s), " . scalar(@proofs) . " proof(s))\n"
      if @proofs && @commands != @proofs;
    ...
    for my $i ( 0 .. $#commands ) {
        ...
        # Only when a proof was actually offered. An announced command has no
        # proof yet BY DESIGN, and checking it for emptiness would refuse the
        # very state TKT-628 exists to create - while a proof that WAS supplied
        # and is blank is still nothing, exactly as TKT-585 made it.
        push @empty, '--proof'
          if @proofs && ( !defined $proof || $proof !~ /\S/ );

        # The closing sentence follows the HALF that is empty, not the status.
        # A blank --proof supplied beside a real command was being told "An
        # empty command announces nothing", which names the half that was
        # fine - and on a non-done call the caller is not announcing, they are
        # supplying a pair with one side blank.
        my $only_command = @empty == 1 && $empty[0] eq '--command';
        die( ( $done ? 'Marking done needs evidence: ' : 'Recording evidence needs it: ' )
          . join( ' and ', @empty )
          . ( @empty == 1 ? ' is empty' : ' are empty' )
          . ( $done ? ". The pair is what says the work happened.\n"
            : $only_command && !@proofs ? ". An empty command announces nothing.\n"
            : ". An empty half is not evidence.\n" ) )
          if @empty;

        # Announced, with no proof yet. The item keeps its status and carries
        # what is being run, which is what the dashboard reads to show it is in
        # progress rather than untouched.
        if ( !@proofs ) {
            push @entries, { command => $command };
            next;
        }
...
sub _log_proof_gate {
    ...
    my @details = map {
        my $what = exists $_->{attachment}
          ...
          # An entry with no proof is an announced command, which has no place
          # in a log of done-claims - the callers above no longer send one. Said
          # rather than interpolated as undef, so a future caller that gets it
          # wrong leaves a legible record instead of a warning and a blank.
...
# What a form needs to know before it can ask for the right fields: which
# parameters a rule requires, and which it refuses outright (card-unassigned
# forbids --column, answer-waiting forbids --age, and so on) - the same
# %POLICY_RULES data policy_add already consults, exposed rather than
# duplicated so a browser-side rule picker cannot drift from what the engine
# actually accepts. TKT-493.
sub policy_rule_specs {
    return {
        map { $_ => { needs => $POLICY_RULES{$_}{needs} // [], forbids => $POLICY_RULES{$_}{forbids} // [] } }
          keys %POLICY_RULES
    };
}
...
# One line each, matching docs/POLICIES.md's own "Saying it in your own
# words" table exactly - a second, hand-copied explanation would drift from
# either the table or %POLICY_MESSAGE_FIELDS the day a token changes. TKT-519:
# asked in the browser dialog itself, where somebody typing --message has no
# other way to find out which {token}s exist.
my %POLICY_MESSAGE_FIELD_HELP = (
...
sub policy_message_field_help { return {%POLICY_MESSAGE_FIELD_HELP} }
...
sub policy_evaluate {
    ...
    for my $policy ( @{$policies} ) {
        ...
        if ( $rule eq 'card-full-details' ) {
            ...
            # TKT-548: mirrors card-changed-by-owner's purpose - the agent
            # might not otherwise notice a tasklist item change - without
            # its mechanism: a tasklist item has no change journal to
            # compare a newest author against, so what it looked like last
            # time police checked is kept instead, in the same store-backed
            # ledger agent-still's own notified-stamp already uses. No
...
# TKT-548's own snapshot, replaced whole rather than merged - an item
# removed from the tasklist (pruned, or its session gone) drops out of the
# ledger the same pass it disappears, rather than lingering as a stale
# entry nothing will ever clean up.
sub _task_changed_mark_seen {
    my ( $self, $store, $next_seen ) = @_;
    return if !$store;
    $self->_with_enforcement_lock( $store, sub {
        my $ledger = $self->_violation_ledger($store);
...
my $live_helpers = $args{live} ? q{const recordsByRef=new Map();const showTitles=document.documentElement.dataset.withTitle==="1";const dialog=document.querySelector(".card-dialog");const sectionsHost=dialog.querySelector(".card-dialog__sections");const errorHost=dialog.querySelector(".card-dialog__error");let lastDialogRecordJson="";let worklogOpen=false;let worklogRefresh=null;const priorityLabels={1:"Low",2:"Medium Low",3:"Medium",4:"High",5:"Very High"};let people=[];const peopleName=id=>{const match=people.find(person=>person.id===id);return match?match.name:id};fetch("/people",{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("people failed");return response.json()}).then(list=>{people=list}).catch(()=>{});let linkTypes=[];fetch("/link-types",{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("link types failed");return response.json()}).then(list=>{linkTypes=list}).catch(()=>{});const el=(tag,cls,text)=>{const node=document.createElement(tag);if(cls)node.className=cls;if(text!==undefined)node.textContent=text;return node};const isDone=entry=>String((entry&&entry.status)||"").toLowerCase()==="done";const isAnnounced=entry=>!isDone(entry)&&Array.isArray(entry&&entry.proof)&&entry.proof.some(e=>e&&e.command&&!e.proof&&!e.attachment);const growBox=(box,cap)=>{const limit=cap||420;const grow=()=>{box.style.height="auto";box.style.height=Math.min(box.scrollHeight,limit)+"px";box.style.overflowY=box.scrollHeight>limit?"auto":"hidden"};box.addEventListener("input",grow);setTimeout(grow,0);return grow};const pasteImageInto=(box,upload)=>{box.addEventListener("paste",event=>{const files=event.clipboardData&&event.clipboardData.files;if(!files||!files.length||!/^image\//.test(files[0].type))return;event.preventDefault();const file=files[0];upload(file);const marker="[image: "+file.name+"]";const start=box.selectionStart||0,end=box.selectionEnd||0;box.value=box.value.slice(0,start)+marker+box.value.slice(end);box.selectionStart=box.selectionEnd=start+marker.length;box.dispatchEvent(new Event("input",{bubbles:true}))})};const mdBarFor=(target)=>{const bar=el("div","card-md-bar");[["bold","B","**"],["italic","I","*"],["code","<>","`"],["list","\u2022","- "]].forEach(spec=>{const control=el("button","card-md-button",spec[1]);control.type="button";control.setAttribute("data-md",spec[0]);control.onclick=()=>{const start=target.selectionStart||0;const end=target.selectionEnd||0;const value=target.value;const selected=value.slice(start,end);if(spec[0]==="list"){const lines=(selected||"item").split("\n").map(line=>"- "+line).join("\n");target.value=value.slice(0,start)+lines+value.slice(end)}else{const mark=spec[2];target.value=value.slice(0,start)+mark+(selected||"text")+mark+value.slice(end)}target.focus();target.dispatchEvent(new Event("input",{bubbles:true}))};bar.appendChild(control)});return bar};const dash="\u2014";const textOr=value=>value===null||value===undefined||value===""?dash:String(value);const entryText=entry=>{if(entry===null||entry===undefined)return dash;if(typeof entry!=="object")return String(entry);if(entry.original_filename)return entry.original_filename+"."+(entry.extension||"bin");return Object.values(entry).filter(value=>typeof value==="string"&&value!=="").join(" \u00b7 ")||dash};const humanDate=value=>value?String(value).replace("T"," ").replace(/[+Z].*$/,""):dash;const showError=message=>{errorHost.textContent=message||"";errorHost.hidden=!message};const viewer=dialog.querySelector(".card-viewer");const imageExts={png:1,jpg:1,jpeg:1,gif:1,webp:1,svg:1};const attachmentUrl=(sha,ext)=>"/attachment?ref="+encodeURIComponent(dialog.dataset.ref)+"&sha="+encodeURIComponent(sha)+"&extension="+encodeURIComponent(ext);const isTextType=entry=>/^text\//.test((entry&&entry.type)||"");const hlLang={html:"markup",xml:"markup",svg:"markup",xhtml:"markup",pl:"perl",pm:"perl",t:"perl",py:"python",rb:"ruby",java:"java",go:"go",rs:"rust",php:"php",js:"cish",jsx:"cish",ts:"cish",tsx:"cish",json:"cish",css:"css",scss:"css",c:"cish",h:"cish",cpp:"cish",hpp:"cish",cc:"cish",sh:"shell",bash:"shell",sql:"sql",yml:"hash",yaml:"hash",toml:"hash",ini:"hash",conf:"hash"};const hlWords={perl:"my our local sub package use require return if elsif else unless while until for foreach last next redo die warn defined undef ref bless wantarray qw eq ne lt gt le ge and or not",python:"def class import from as return if elif else while for in is not and or try except finally raise with lambda None True False pass yield global nonlocal assert del",ruby:"def class module require return if elsif else unless while until for in do end begin rescue ensure raise yield nil true false self and or not attr_accessor",java:"public private protected class interface extends implements static final void new return if else while for switch case break continue try catch finally throw throws import package this null true false",go:"func package import type struct interface map chan go defer return if else for range switch case break continue var const nil true false",rust:"fn let mut struct enum impl trait use mod pub match if else while for loop return break continue Some None Ok Err self crate where",php:"function class public private protected static return if elseif else foreach while for switch case break continue try catch finally throw new echo print require include namespace use null true false",cish:"function return if else while for switch case break continue new delete class struct typedef const static void int char float double let var typedef import export default async await try catch finally throw null true false undefined typeof instanceof",css:"import media supports keyframes font-face charset important inherit initial unset none auto",shell:"if then elif else fi for while do done case esac function return local export readonly declare source echo exit trap set unset",sql:"select from where join inner left right outer on group by order having insert update delete into values set create table alter drop index primary key foreign not null and or as distinct limit",hash:"true false null yes no on off"};const hlComment={perl:"#",python:"#",ruby:"#",shell:"#",hash:"#",sql:"--",java:"//",go:"//",rust:"//",php:"//",cish:"//",css:null};const hlWordSet=(()=>{const out={};for(const k in hlWords){out[k]=Object.create(null);for(const w of hlWords[k].split(" "))out[k][w]=1}return out})();const highlight=(text,lang)=>{const frag=document.createDocumentFragment();if(lang==="markup"){let i=0,plain="";const push=(cls,value)=>{if(!value)return;const node=cls?document.createElement("span"):document.createTextNode(value);if(cls){node.className="tok tok--"+cls;node.textContent=value}frag.appendChild(node)};const flush=()=>{push(null,plain);plain=""};while(i<text.length){const rest=text.slice(i);if(rest.startsWith("<!--")){const end=rest.indexOf("-->");const take=end<0?rest:rest.slice(0,end+3);flush();push("comment",take);i+=take.length;continue}let m=rest.match(/^<\/?[A-Za-z][\w:.-]*/);if(m){flush();push("keyword",m[0]);i+=m[0].length;continue}m=rest.match(/^"[^"]*"|^'[^']*'/);if(m){flush();push("string",m[0]);i+=m[0].length;continue}plain+=rest[0];i++}flush();return frag}const words=hlWordSet[lang];if(!words){frag.appendChild(document.createTextNode(text));return frag}const line=hlComment[lang];const push=(cls,value)=>{if(!value)return;const node=cls?document.createElement("span"):document.createTextNode(value);if(cls){node.className="tok tok--"+cls;node.textContent=value}frag.appendChild(node)};let i=0,plain="";const flush=()=>{push(null,plain);plain=""};while(i<text.length){const rest=text.slice(i);let m;if(line&&rest.startsWith(line)){m=rest.match(/^[^\n]*/);flush();push("comment",m[0]);i+=m[0].length;continue}if(rest.startsWith("/*")){const end=rest.indexOf("*/");const take=end<0?rest:rest.slice(0,end+2);flush();push("comment",take);i+=take.length;continue}const q=rest[0];if(q==='"'||q==="'"||q==="`"){let j=1;while(j<rest.length){if(rest[j]==="\\\\"){j+=2;continue}if(rest[j]===q){j++;break}j++}flush();push("string",rest.slice(0,j));i+=j;continue}if((m=rest.match(/^\d[\d._]*(?:[eE][+-]?\d+)?/))){flush();push("number",m[0]);i+=m[0].length;continue}if((m=rest.match(/^[A-Za-z_$][\w$]*/))){const w=m[0];if(words[w]){flush();push("keyword",w)}else{plain+=w}i+=w.length;continue}plain+=rest[0];i++;}flush();return frag;};const videoExts={mp4:1,m4v:1,mov:1,webm:1};const audioExts={mp3:1,wav:1,m4a:1,ogg:1,flac:1};const tiffExts={tif:1,tiff:1};const docExts={pdf:1};const viewerPanes=()=>({frame:viewer.querySelector("iframe"),image:viewer.querySelector("img"),textPane:viewer.querySelector(".card-viewer__text"),video:viewer.querySelector(".card-viewer__video"),audio:viewer.querySelector(".card-viewer__audio"),fallback:viewer.querySelector(".card-viewer__fallback")});const hideAllPanes=()=>{const panes=viewerPanes();panes.frame.hidden=true;panes.frame.src="about:blank";panes.image.hidden=true;panes.image.removeAttribute("src");panes.image.onerror=null;panes.textPane.hidden=true;panes.textPane.textContent="";panes.video.hidden=true;panes.video.pause();panes.video.removeAttribute("src");panes.video.load();panes.audio.hidden=true;panes.audio.pause();panes.audio.removeAttribute("src");panes.fallback.hidden=true};let viewerList=[];let viewerIndex=-1;const updateViewerNav=()=>{viewer.querySelector(".card-viewer__prev").disabled=viewerIndex<=0;viewer.querySelector(".card-viewer__next").disabled=viewerIndex<0||viewerIndex>=viewerList.length-1};const renderViewerEntry=()=>{const entry=viewerList[viewerIndex];if(!entry)return;const sha=entry.sha;const ext=entry.ext;const name=entry.name;const panes=viewerPanes();const download=viewer.querySelector(".card-viewer__download");viewer.querySelector(".card-viewer__name").textContent=name;const url=attachmentUrl(sha,ext);download.href=url;download.setAttribute("download",name);hideAllPanes();if(imageExts[ext]||tiffExts[ext]){panes.image.onerror=()=>{panes.image.hidden=true;panes.fallback.hidden=false};panes.image.src=url;panes.image.hidden=false}else if(videoExts[ext]){panes.video.src=url;panes.video.hidden=false}else if(audioExts[ext]){panes.audio.src=url;panes.audio.hidden=false}else if(isTextType(entry)){panes.textPane.textContent="Loading\u2026";panes.textPane.hidden=false;fetch(url,{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("attachment failed");return response.text()}).then(content=>{panes.textPane.textContent="";panes.textPane.appendChild(highlight(content,hlLang[ext]||""))}).catch(()=>{panes.textPane.textContent="Unable to load attachment"})}else if(docExts[ext]){panes.frame.src=url;panes.frame.hidden=false}else{panes.fallback.hidden=false}viewer.hidden=false;updateViewerNav()};const openViewer=(list,index)=>{viewerList=list||[];viewerIndex=index;renderViewerEntry()};const closeViewer=()=>{viewer.hidden=true;hideAllPanes();viewerList=[];viewerIndex=-1};viewer.querySelector(".card-viewer__close").addEventListener("click",closeViewer);viewer.querySelector(".card-viewer__prev").addEventListener("click",()=>{if(viewerIndex>0){viewerIndex--;renderViewerEntry()}});viewer.querySelector(".card-viewer__next").addEventListener("click",()=>{if(viewerIndex<viewerList.length-1){viewerIndex++;renderViewerEntry()}});dialog.addEventListener("cancel",event=>{if(!viewer.hidden){event.preventDefault();closeViewer()}});dialog.addEventListener("close",()=>{closeViewer();showError("");cardNavStack=[];updateBackButton()});const uploadFile=(file,commentId,quiet)=>{const reader=new FileReader();reader.onload=()=>{const base64=String(reader.result).split(",")[1]||"";const payload={ref:dialog.dataset.ref,filename:file.name,content_base64:base64};if(commentId)payload.comment=commentId;mutate("/attachment/add",payload,quiet)};reader.readAsDataURL(file)};const inlineMd=(host,text)=>{const pattern=/(\*\*[^*]+\*\*|\*[^*]+\*|`[^`]+`)/g;let last=0;let match;while((match=pattern.exec(text))){if(match.index>last)host.appendChild(document.createTextNode(text.slice(last,match.index)));const token=match[0];if(token.indexOf("**")===0)host.appendChild(el("strong","",token.slice(2,-2)));else if(token.indexOf("`")===0)host.appendChild(el("code","card-md-code",token.slice(1,-1)));else host.appendChild(el("em","",token.slice(1,-1)));last=match.index+token.length}if(last<text.length)host.appendChild(document.createTextNode(text.slice(last)))};const renderMarkdown=(body,cls)=>{const container=el("div",cls||"card-comment__body");String(body||"").split(/\n{2,}/).forEach(block=>{const lines=block.split("\n");if(lines.length&&lines.every(line=>/^\s*[-*] /.test(line))){const listNode=el("ul","card-md-list");lines.forEach(line=>{const item=el("li","");inlineMd(item,line.replace(/^\s*[-*] /,""));listNode.appendChild(item)});container.appendChild(listNode)}else{const paragraph=el("p","card-md-p");lines.forEach((line,index)=>{if(index)paragraph.appendChild(document.createElement("br"));inlineMd(paragraph,line)});container.appendChild(paragraph)}});return container};const sortByStamp=(items,key)=>[...(items||[])].sort((a,b)=>String(b[key]||"").localeCompare(String(a[key]||"")));const attachChip=(reference,commentId,list,index)=>{const chip=el("span","card-attachment"+(reference.discarded_at?" card-attachment--discarded":""));const key=reference.sha+"."+reference.extension;const name=reference.original_filename||key;const view=el("button","card-attachment__view",name);view.appendChild(el("span","card-attachment__date",humanDate(reference.added_at)));view.type="button";view.dataset.viewAttachment=key;view.onclick=()=>openViewer(list,index);const drop=el("button","card-attachment__delete","\u00d7");drop.type="button";drop.dataset.detachAttachment=key;drop.title=reference.discarded_at?("Discarded "+humanDate(reference.discarded_at)+(reference.discarded_by?" by "+reference.discarded_by:"")):"Discard attachment";drop.disabled=!!reference.discarded_at;drop.onclick=()=>{if(!confirm("Discard "+name+"?"))return;const payload={ref:dialog.dataset.ref,sha:reference.sha,extension:reference.extension};if(commentId)payload.comment=commentId;mutate("/attachment/discard",payload)};chip.append(view,drop);return chip};const attachInput=commentId=>{const label=el("label","card-attach-add");label.append(el("span","","Attach file"));const input=el("input","card-attach-input");input.type="file";if(commentId){input.dataset.attachComment=commentId}else{input.dataset.attachTarget="record"}input.onchange=()=>{if(input.files[0])uploadFile(input.files[0],commentId)};label.appendChild(input);return label};const listFields={labels:1,affects_versions:1,key_details:1,deliverables:1,scope_included:1,scope_excluded:1,acceptance_criteria:1,test_steps:1,bdd:1,atdd:1};const sendList=(field,items)=>mutate("/update",{ref:dialog.dataset.ref,field:field,value:items});const editableList=(field,items)=>{const wrap=el("div","card-list-wrap");const list=el("ul","card-list card-list--editable");(items||[]).forEach((item,index)=>{const row=el("li","card-list__row");const text=el("span","card-list__text",String(item));const editBtn=el("button","card-list__action","\u270e");editBtn.type="button";editBtn.dataset.listEdit=field+":"+index;editBtn.title="Edit item";editBtn.onclick=()=>{const input=el("input","card-edit-input");input.type="text";input.value=String(item);input.dataset.listInput=field;const save=el("button","card-edit-save","Save");save.type="button";save.dataset.listSave=field;save.onclick=()=>{const next=(items||[]).map(String);next[index]=input.value;sendList(field,next)};const cancel=el("button","card-edit-cancel","Cancel");cancel.type="button";cancel.onclick=()=>reloadCard();const editor=el("span","card-edit");editor.append(input,save,cancel);row.replaceChildren(editor);input.focus()};const removeBtn=el("button","card-list__action card-list__action--danger","\u00d7");removeBtn.type="button";removeBtn.dataset.listRemove=field+":"+index;removeBtn.title="Remove item";removeBtn.onclick=()=>{const next=(items||[]).map(String);next.splice(index,1);sendList(field,next)};row.append(text,editBtn,removeBtn);list.appendChild(row)});wrap.appendChild(list);const adder=el("div","card-list__adder");const input=el("input","card-edit-input");input.type="text";input.placeholder="Add item";input.setAttribute("data-list-add",field);const save=el("button","card-edit-save","Add");save.type="button";save.dataset.listAddSave=field;const submit=()=>{if(!input.value)return;sendList(field,(items||[]).map(String).concat(input.value))};save.onclick=submit;input.onkeydown=event=>{if(event.key==="Enter"){event.preventDefault();submit()}};adder.append(input,save);wrap.appendChild(adder);return wrap};const reloadCard=()=>fetch("/record?type="+encodeURIComponent(dialog.dataset.type)+"&ref="+encodeURIComponent(dialog.dataset.ref),{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("detail failed");return response.json()}).then(record=>{renderCard(record);renderQuestions(record);renderPoliceLog(record);renderWorkLog(record);return record}).catch(()=>null);const dialogEditingActive=()=>!!(dialog.querySelector(".card-new")||dialog.querySelector(".card-edit")||dialog.querySelector(".card-comment__edit")||(document.activeElement&&dialog.contains(document.activeElement)&&/^(INPUT|TEXTAREA|SELECT)$/.test(document.activeElement.tagName))||(dialog.querySelector(".card-comment-form")&&!dialog.querySelector(".card-comment-form").hidden));const maybeRefreshDialog=()=>{if(!dialog.open)return;if(!dialog.dataset.ref)return;if(dialogEditingActive())return;fetch("/record?type="+encodeURIComponent(dialog.dataset.type)+"&ref="+encodeURIComponent(dialog.dataset.ref),{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("detail failed");return response.json()}).then(record=>{if(!record||!dialog.open||dialogEditingActive())return;if(JSON.stringify(record)===lastDialogRecordJson){if(worklogRefresh)worklogRefresh();return}renderCard(record);renderQuestions(record);renderPoliceLog(record);renderWorkLog(record)}).catch(()=>null)};const mutate=(path,payload,quiet)=>fetch(path,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}).then(response=>response.json()).then(result=>{window.__tiraLastMutation=path;window.__tiraMutationSeq=(window.__tiraMutationSeq||0)+1;if(result.ok){showError("");return quiet?null:reloadCard()}if(result.conflict){showError(result.error||"This card changed while you were editing \u2014 review and retry");return reloadCard()}showError(result.error||"Change failed");return null}).catch(()=>{window.__tiraLastMutation=path;window.__tiraMutationSeq=(window.__tiraMutationSeq||0)+1;showError("Change failed");return null});const longFields={description:1,problem_or_feature:1,solution_needed:1};const editorFor=field=>{if(field==="priority"){const select=el("select","card-edit-input");["","1","2","3","4","5"].forEach(value=>{const option=el("option","",value===""?dash:value+" "+priorityLabels[value]);option.value=value;select.appendChild(option)});return select}if(field==="assignee"||field==="reporter"){const select=el("select","card-edit-input");const empty=el("option","",dash);empty.value="";select.appendChild(empty);people.forEach(person=>{const option=el("option","",person.name);option.value=person.id;select.appendChild(option)});return select}if(longFields[field]){const area=el("textarea","card-edit-input");area.rows=5;return area}const input=el("input","card-edit-input");input.type="text";return input};const beginEdit=(field,current,slot)=>{const editor=editorFor(field);editor.value=current===null||current===undefined?"":String(current);const base=current===undefined?null:current;const save=el("button","card-edit-save","Save");save.type="button";save.dataset.save=field;save.onclick=()=>mutate("/update",{ref:dialog.dataset.ref,field:field,value:editor.value,base:base});const cancel=el("button","card-edit-cancel","Cancel");cancel.type="button";cancel.onclick=()=>reloadCard();const wrap=el("span","card-edit");if(longFields[field]){wrap.classList.add("card-edit--long");slot.classList.add("card-value--long");wrap.append(mdBarFor(editor));growBox(editor);pasteImageInto(editor,file=>uploadFile(file,undefined,true))}wrap.append(editor,save,cancel);slot.replaceChildren(wrap);editor.focus()};const editableValue=(field,record,display)=>{const slot=el("span","card-value");slot.appendChild(el("span","card-value__text",display));const edit=el("button","card-edit-button","\u270e");edit.type="button";edit.dataset.edit=field;edit.title="Edit "+field.replace(/_/g," ");edit.onclick=()=>beginEdit(field,record[field],slot);slot.appendChild(edit);return slot};const section=(title,body)=>{const box=el("section","card-section");box.dataset.section=title.toLowerCase();box.appendChild(el("h3","card-section__title",title));box.appendChild(body);return box};const listBody=values=>{const list=el("ul","card-list");values.forEach(value=>list.appendChild(el("li","",value)));return list};const maybeListSection=(title,items,map)=>{const values=(items||[]).map(map||entryText);if(!values.length)return;sectionsHost.appendChild(section(title,listBody(values)))};const paginatedListSection=(title,items,pageSize)=>{const values=(items||[]).map(entryText);if(!values.length)return;const list=el("ul","card-list");let shown=0;const more=el("button","card-list__more","Load more");more.type="button";const showMore=()=>{const next=values.slice(shown,shown+pageSize);next.forEach(value=>list.appendChild(el("li","",value)));shown+=next.length;more.hidden=shown>=values.length;if(!more.hidden)more.textContent="Load more ("+(values.length-shown)+" more)"};showMore();more.onclick=showMore;const wrap=el("div","card-list-wrap");wrap.append(list,more);sectionsHost.appendChild(section(title,wrap))};const detailRow=(grid,label,value)=>{grid.appendChild(el("dt","",label));const dd=el("dd","");if(value instanceof Node){dd.appendChild(value)}else{dd.textContent=value}grid.appendChild(dd)};const renderCard=record=>{if(!record)return;lastDialogRecordJson=JSON.stringify(record);dialog.dataset.ref=record.ref;dialog.dataset.type=record.type;{const refLine=dialog.querySelector(".card-dialog__ref");refLine.replaceChildren(el("span","",[record.ref||"Card",record.type].filter(Boolean).join(" \u00b7 ")+" \u00b7 "));const statusSelect=el("select","card-status");statusSelect.title="Move to column";const board=document.querySelector(".board--"+record.type);if(board){const lists=[...board.querySelectorAll(".cards")];const headers=[...board.querySelectorAll(".column__head")];lists.forEach((list,index)=>{const header=headers[index];const headerName=header?header.querySelector(".column__name"):null;const option=el("option","",headerName?headerName.textContent:(header?header.textContent:list.dataset.column));option.value=list.dataset.column;if(list.dataset.column===record.column)option.selected=true;statusSelect.appendChild(option)})}else{const option=el("option","",record.column||"");option.value=record.column||"";option.selected=true;statusSelect.appendChild(option)}statusSelect.onchange=()=>{fetch("/move",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:record.type,ref:record.ref,column:statusSelect.value})}).then(response=>{if(!response.ok)throw new Error("move failed");return response.json()}).then(()=>{window.__tiraMutationSeq=(window.__tiraMutationSeq||0)+1;refreshDashboard();return reloadCard()}).catch(()=>showError("Unable to move the card"))};refLine.appendChild(statusSelect)};dialog.querySelector(".card-dialog__title").replaceChildren(editableValue("title",record,textOr(record.title)));sectionsHost.replaceChildren();const grid=el("dl","card-details");detailRow(grid,"Assignee",editableValue("assignee",record,record.assignee?peopleName(record.assignee):dash));detailRow(grid,"Reporter",editableValue("reporter",record,record.reporter?peopleName(record.reporter):dash));detailRow(grid,"Priority",editableValue("priority",record,record.priority?record.priority+" "+priorityLabels[record.priority]:dash));detailRow(grid,"Labels",editableList("labels",record.labels));detailRow(grid,"Start date",editableValue("start_date",record,humanDate(record.start_date)));detailRow(grid,"Due date",editableValue("due_date",record,humanDate(record.due_date)));detailRow(grid,"Fix version",editableValue("fix_version",record,textOr(record.fix_version)));detailRow(grid,"Affects versions",editableList("affects_versions",record.affects_versions));detailRow(grid,"SDLC gate",editableValue("sdlc_gate",record,textOr(record.sdlc_gate)));detailRow(grid,"Lifecycle",editableValue("lifecycle",record,textOr(record.lifecycle)));detailRow(grid,"Parent",textOr(record.parent));detailRow(grid,"Source",editableValue("source",record,textOr(record.source)));detailRow(grid,"Created",humanDate(record.created_at));detailRow(grid,"Last updated",humanDate(record.last_updated));sectionsHost.appendChild(section("Details",grid));const sectionWithEdit=(title,field)=>{const box=el("section","card-section");const heading=el("h3","card-section__title",title);const edit=el("button","card-edit-button","\u270e");edit.type="button";edit.dataset.edit=field;edit.title="Edit "+title;heading.appendChild(edit);const body=el("div","card-text");const slot=el("span","card-value");slot.appendChild(longFields[field]&&record[field]?renderMarkdown(record[field],"card-md-body"):el("span","card-value__text",textOr(record[field])));body.appendChild(slot);edit.onclick=()=>beginEdit(field,record[field],slot);box.append(heading,body);return box};[["Description","description"],["Problem / Feature","problem_or_feature"],["Solution Needed","solution_needed"]].forEach(pair=>sectionsHost.appendChild(sectionWithEdit(pair[0],pair[1])));[["Key Details","key_details",record.key_details],["Deliverables","deliverables",record.deliverables],["Scope Included","scope_included",record.scope&&record.scope.included],["Scope Excluded","scope_excluded",record.scope&&record.scope.excluded],["Acceptance Criteria","acceptance_criteria",record.acceptance_criteria],["Test Steps","test_steps",record.test_steps],["BDD","bdd",record.bdd],["ATDD","atdd",record.atdd]].forEach(triple=>sectionsHost.appendChild(section(triple[0],editableList(triple[1],triple[2]))));{const box=el("div","card-checklist");const list=el("ul","card-list");(record.checklist||[]).forEach(entry=>{const row=el("li","card-list__row");row.dataset.checklist=entry.id;const text=el("span","card-list__text","["+(entry.status||"open")+"] "+(entry.item||dash));const editBtn=el("button","card-list__action","\u270e");editBtn.type="button";editBtn.dataset.checklistEdit=entry.id;editBtn.title="Edit checklist entry";editBtn.onclick=()=>{const itemInput=el("input","card-edit-input");itemInput.type="text";itemInput.value=entry.item||"";itemInput.dataset.checklistItem=entry.id;const statusInput=el("input","card-edit-input");statusInput.type="text";statusInput.value=entry.status||"";statusInput.dataset.checklistStatus=entry.id;const save=el("button","card-edit-save","Save");save.type="button";save.dataset.checklistSave=entry.id;save.onclick=()=>mutate("/checklist/update",{ref:dialog.dataset.ref,id:entry.id,item:itemInput.value,status:statusInput.value});const cancel=el("button","card-edit-cancel","Cancel");cancel.type="button";cancel.onclick=()=>reloadCard();const editor=el("span","card-edit");editor.append(itemInput,statusInput,save,cancel);row.replaceChildren(editor);statusInput.focus()};row.append(text,editBtn);list.appendChild(row)});box.appendChild(list);const form=el("form","card-checklist-form");const itemInput=el("input","");itemInput.name="item";itemInput.placeholder="New checklist item";const statusInput=el("input","");statusInput.name="status";statusInput.placeholder="Status";const submit=el("button","","Add entry");submit.type="submit";form.append(itemInput,statusInput,submit);form.onsubmit=event=>{event.preventDefault();if(!itemInput.value||!statusInput.value)return;mutate("/checklist/add",{ref:dialog.dataset.ref,item:itemInput.value,status:statusInput.value})};box.appendChild(form);sectionsHost.appendChild(section("Checklist",box))};if((record.required_items||[]).length){const box=el("div","card-required");const total=record.required_items.length;const done=record.required_items.filter(isDone).length;const exemptByItem=new Map((record.required_exempt||[]).map(entry=>typeof entry==="object"&&entry!==null?[entry.item,entry.reason]:[entry,null]));const byColumn=new Map();record.required_items.forEach(entry=>{const col=entry.column||dash;if(!byColumn.has(col))byColumn.set(col,[]);byColumn.get(col).push(entry)});byColumn.forEach((entries,col)=>{const group=el("div","card-required__group");group.appendChild(el("h4","card-required__column",col));const list=el("ul","card-list");entries.forEach(entry=>{const row=el("li","card-list__row");row.dataset.requiredAction=entry.id;const exempt=exemptByItem.has(entry.item);const icon=exempt?"\u2796":isDone(entry)?"\u2705":isAnnounced(entry)?"\u23f1":"\u2b1c";const textSpan=el("span","card-list__text",icon+" "+humanDate(entry.last_updated)+" "+(entry.item||dash));if(exempt)textSpan.classList.add("card-list__text--exempt");if(entry.repeated_reason){row.classList.add("card-list__row--repeated");textSpan.title="Same evidence as another item in this column - reason: "+entry.repeated_reason}row.appendChild(textSpan);if(!isDone(entry)&&!exempt){const doneCheck=document.createElement("input");doneCheck.type="checkbox";doneCheck.className="card-list__check";doneCheck.dataset.requiredActionDone=entry.id;doneCheck.onclick=()=>mutate("/required-action/update",{ref:dialog.dataset.ref,id:entry.id,status:"done"});row.appendChild(doneCheck)}list.appendChild(row);if(exempt){textSpan.classList.add("card-list__text--expandable");textSpan.tabIndex=0;textSpan.setAttribute("role","button");textSpan.setAttribute("aria-expanded","false");const openExempt=()=>{const proofDialog=document.querySelector(".proof-dialog");const body=proofDialog.querySelector(".proof-dialog__body");body.textContent="";const pairBox=el("div","proof-dialog__pair");pairBox.appendChild(el("div","proof-dialog__command","Exempt"));pairBox.appendChild(el("div","proof-dialog__detail",exemptByItem.get(entry.item)||"(no reason recorded)"));body.appendChild(pairBox);textSpan.setAttribute("aria-expanded","true");proofDialog.addEventListener("close",()=>textSpan.setAttribute("aria-expanded","false"),{once:true});proofDialog.showModal()};textSpan.onclick=openExempt;textSpan.onkeydown=keyEvent=>{if(keyEvent.key==="Enter"||keyEvent.key===" "){keyEvent.preventDefault();openExempt()}}}else if(entry.proof&&entry.proof.length){textSpan.classList.add("card-list__text--expandable");textSpan.tabIndex=0;textSpan.setAttribute("role","button");textSpan.setAttribute("aria-expanded","false");const proofRow=el("li","card-list__proof");proofRow.hidden=true;proofRow.dataset.requiredActionProof=entry.id;entry.proof.forEach(pair=>{const line=el("div","card-list__proof-line");line.appendChild(el("span","card-list__proof-command",pair.command||dash));line.appendChild(el("span","card-list__proof-detail",pair.proof||(pair.attachment?"(attachment)":dash)));proofRow.appendChild(line)});const openProof=()=>{const proofDialog=document.querySelector(".proof-dialog");const body=proofDialog.querySelector(".proof-dialog__body");body.textContent="";if(entry.repeated_reason){const why=el("div","proof-dialog__repeated");why.appendChild(el("span","proof-dialog__repeated-label","Same evidence as another item - reason given:"));why.appendChild(el("span","proof-dialog__repeated-text",entry.repeated_reason));body.appendChild(why)}entry.proof.forEach(pair=>{const pairBox=el("div","proof-dialog__pair");pairBox.appendChild(el("div","proof-dialog__command",pair.command||dash));pairBox.appendChild(el("div","proof-dialog__detail",pair.proof||(pair.attachment?"(attachment)":dash)));body.appendChild(pairBox)});textSpan.setAttribute("aria-expanded","true");proofDialog.addEventListener("close",()=>textSpan.setAttribute("aria-expanded","false"),{once:true});proofDialog.showModal()};textSpan.onclick=openProof;textSpan.onkeydown=keyEvent=>{if(keyEvent.key==="Enter"||keyEvent.key===" "){keyEvent.preventDefault();openProof()}};list.appendChild(proofRow)}});group.appendChild(list);box.appendChild(group)});const requiredSection=section("Required actions ("+done+"/"+total+")",box);requiredSection.classList.add("card-section--required");sectionsHost.appendChild(requiredSection)}maybeListSection("Subtasks",record.subtasks);if(record.linkage){const box=el("div","card-linkage");const linkedCache=new Map();const priorityRank=info=>info&&typeof info.priority==="number"?info.priority:-1;const linkedInfo=ref=>{if(linkedCache.has(ref))return linkedCache.get(ref);const seed=recordsByRef.get(ref);const promise=fetch("/record?type="+encodeURIComponent(dialog.dataset.type)+"&ref="+encodeURIComponent(ref),{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("linked record failed");return response.json()}).catch(()=>seed||null);linkedCache.set(ref,promise);return promise};const sortLinkageTable=table=>{[...table.children].sort((a,b)=>(Number(b.dataset.priority??-1)-Number(a.dataset.priority??-1))||(a.getAttribute("data-linkage-row")||"").localeCompare(b.getAttribute("data-linkage-row")||"")).forEach(row=>table.appendChild(row))};const linkageRow=(ref,drop)=>{const row=el("div","card-linkage-table__row");row.setAttribute("data-linkage-row",ref);const titleCell=el("span","card-linkage__title","\u2026");const statusCell=el("span","card-linkage__status","");row.append(el("span","card-linkage__ref",ref),titleCell,statusCell);if(drop)row.appendChild(drop);row.addEventListener("click",event=>{if(event.target.closest("[data-linkage-unlink],[data-link-remove]"))return;navigateToCard(ref)});linkedInfo(ref).then(info=>{titleCell.textContent=info&&info.title?info.title:dash;statusCell.textContent=info&&info.column?info.column:"";row.dataset.priority=priorityRank(info);if(row.parentElement&&row.parentElement.classList.contains("card-linkage-table"))sortLinkageTable(row.parentElement)});return row};const linkRoutes={hierarchy:{link:"/hierarchy/link",unlink:"/hierarchy/unlink"},subitem:{link:"/subitem/link",unlink:"/subitem/unlink"}};const linkageSpec={sow_ref:"hierarchy",epic_ref:"hierarchy",parent_sow_ref:"subitem",parent_epic_ref:"subitem",parent_ticket_ref:"subitem",epic_refs:"hierarchy",ticket_refs:"hierarchy",sub_sow_refs:"subitem",sub_epic_refs:"subitem",sub_ticket_refs:"subitem"};Object.keys(record.linkage).forEach(key=>{if(key==="links")return;const kind=linkageSpec[key];if(!kind)return;const value=record.linkage[key];const row=el("div","card-linkage__row");row.appendChild(el("span","card-linkage__label",key.replace(/_/g," ")));const holder=el("span","card-linkage__list");if(Array.isArray(value)){const table=el("div","card-linkage-table");value.forEach(ref=>{const drop=el("button","card-list__action card-list__action--danger","\u00d7");drop.type="button";drop.setAttribute("data-linkage-unlink",key+":"+ref);drop.title="Unlink";drop.onclick=()=>mutate(linkRoutes[kind].unlink,{parent:dialog.dataset.ref,child:ref});table.appendChild(linkageRow(ref,drop))});holder.appendChild(table);const input=el("input","card-edit-input");input.type="text";input.placeholder="REF";input.setAttribute("data-linkage-add",key);const add=el("button","card-edit-save","Link");add.type="button";add.setAttribute("data-linkage-add-save",key);add.onclick=()=>{if(input.value)mutate(linkRoutes[kind].link,{parent:dialog.dataset.ref,child:input.value})};holder.append(input,add)}else if(value){const table=el("div","card-linkage-table");const drop=el("button","card-list__action card-list__action--danger","Unlink");drop.type="button";drop.setAttribute("data-linkage-unlink",key+":"+String(value));drop.onclick=()=>mutate(linkRoutes[kind].unlink,{parent:String(value),child:dialog.dataset.ref});table.appendChild(linkageRow(String(value),drop));holder.appendChild(table)}else{const input=el("input","card-edit-input");input.type="text";input.placeholder="Parent REF";input.setAttribute("data-linkage-add",key);const add=el("button","card-edit-save","Link");add.type="button";add.setAttribute("data-linkage-add-save",key);add.onclick=()=>{if(input.value)mutate(linkRoutes[kind].link,{parent:input.value,child:dialog.dataset.ref})};holder.append(input,add)}row.appendChild(holder);box.appendChild(row)});const linksBox=el("div","card-linkage__links");const linkList=el("div","card-linkage-table");(record.linkage.links||[]).forEach(entry=>{const drop=el("button","card-list__action card-list__action--danger","\u00d7");drop.type="button";drop.setAttribute("data-link-remove",entry.type+":"+entry.ref);drop.title="Remove link";drop.onclick=()=>mutate("/link/remove",{from:dialog.dataset.ref,type:entry.type,to:entry.ref});const row=linkageRow(entry.ref,drop);row.classList.add("card-linkage-table__row--typed");row.insertBefore(el("span","card-linkage__type",entry.type),row.firstChild);linkList.appendChild(row)});linksBox.appendChild(linkList);const form=el("form","card-link-form");const select=el("select","");select.name="type";linkTypes.forEach(pair=>{[pair.outward,pair.inward].forEach(nameValue=>{if([...select.options].some(option=>option.value===nameValue))return;const option=el("option","",nameValue);option.value=nameValue;select.appendChild(option)})});const refInput=el("input","");refInput.name="to";refInput.placeholder="REF";const submit=el("button","","Add link");submit.type="submit";form.append(select,refInput,submit);form.onsubmit=event=>{event.preventDefault();if(!refInput.value)return;mutate("/link/add",{from:dialog.dataset.ref,type:select.value,to:refInput.value})};linksBox.appendChild(form);box.appendChild(linksBox);sectionsHost.appendChild(section("Linkage",box))}paginatedListSection("Gate Passing Log",record.gate_passing_log,10);maybeListSection("Evidence",record.evidence);{const box=el("div","card-attachments");const strip=el("div","card-attachment-strip");sortByStamp(record.attachments,"added_at").forEach((reference,index,arr)=>strip.appendChild(attachChip(reference,undefined,arr.map(r=>({sha:r.sha,ext:r.extension,type:r.content_type||"",name:r.original_filename||(r.sha+"."+r.extension)})),index)));box.appendChild(strip);box.appendChild(attachInput());sectionsHost.appendChild(section("Attachments",box))};const comments=el("div","card-comments-box");const commentList=el("ul","card-comments");sortByStamp(record.comments,"created_at").forEach(comment=>{const item=el("li","card-comment");item.dataset.comment=comment.id;const head=el("div","card-comment__head");head.appendChild(el("strong","",peopleName(comment.author)));head.appendChild(el("span","card-comment__meta",comment.id+" \u00b7 "+humanDate(comment.created_at)+(comment.last_updated&&comment.last_updated!==comment.created_at?" \u00b7 edited "+humanDate(comment.last_updated):"")));const editButton=el("button","card-comment__action","Edit");editButton.type="button";editButton.dataset.commentEdit=comment.id;const removeButton=el("button","card-comment__action card-comment__action--danger","Delete");removeButton.type="button";removeButton.dataset.commentRemove=comment.id;head.append(editButton,removeButton);const body=comment.format==="text"?el("div","card-comment__body",comment.body):renderMarkdown(comment.body);item.append(head,body);const commentAttachments=el("div","card-comment__attachments");sortByStamp(comment.attachments,"added_at").forEach((reference,index,arr)=>commentAttachments.appendChild(attachChip(reference,comment.id,arr.map(r=>({sha:r.sha,ext:r.extension,type:r.content_type||"",name:r.original_filename||(r.sha+"."+r.extension)})),index)));commentAttachments.appendChild(attachInput(comment.id));item.appendChild(commentAttachments);editButton.onclick=()=>{const area=el("textarea","card-comment__editor");area.value=comment.body;growBox(area);const save=el("button","card-comment__action","Save");save.type="button";save.dataset.commentSave=comment.id;save.onclick=()=>mutate("/comment/update",{ref:dialog.dataset.ref,comment:comment.id,text:area.value});const cancel=el("button","card-comment__action","Cancel");cancel.type="button";cancel.onclick=()=>reloadCard();const editor=el("div","card-comment__edit");editor.append(mdBarFor(area),area,save,cancel);body.replaceChildren(editor)};removeButton.onclick=()=>mutate("/comment/remove",{ref:dialog.dataset.ref,comment:comment.id});commentList.appendChild(item)});const composer=el("div","card-composer");const toggle=el("button","card-composer-toggle","\u002b Add a comment");toggle.type="button";const form=el("form","card-comment-form");form.hidden=true;const text=el("textarea","");text.name="text";text.rows=4;text.placeholder="Write a comment - basic markdown supported";growBox(text);const bar=mdBarFor(text);const submit=el("button","","Comment");submit.type="submit";form.append(bar,text,submit);toggle.onclick=()=>{toggle.hidden=true;form.hidden=false;text.focus()};form.onsubmit=event=>{event.preventDefault();if(!text.value)return;mutate("/comment/add",{ref:dialog.dataset.ref,text:text.value})};composer.append(toggle,form);comments.appendChild(composer);comments.appendChild(commentList);sectionsHost.appendChild(section("Comments",comments))};const questionRank=question=>question.discarded_at?3:!question.answer?0:!question.answer.mark?1:2;const renderPoliceLog=record=>{const host=sectionsHost;if(!host)return;const box=el("section","card-section card-section--policelog");box.dataset.section="policelog";box.hidden=true;const head=el("h3","card-section__title","What police has said");const list=el("ol","card-policelog__list");box.append(head,list);host.appendChild(box);fetch("/policelog?ref="+encodeURIComponent(record.ref),{cache:"no-store"}).then(r=>{if(!r.ok)throw new Error("police log failed");return r.json()}).then(entries=>{if(!entries||!entries.length){box.hidden=true;return}head.textContent="What police has said ("+entries.length+")";entries.forEach(entry=>{const item=el("li","card-policelog__entry");item.appendChild(el("span","card-policelog__when",humanDate(entry.at)));item.appendChild(el("span","card-policelog__kind",entry.kind||dash));item.appendChild(el("span","card-policelog__detail",entry.detail||dash));list.appendChild(item)});box.hidden=false}).catch(()=>{box.hidden=true})};const renderWorkLog=record=>{const host=sectionsHost;if(!host)return;const box=el("section","card-section card-section--worklog");box.dataset.section="worklog";const head=el("button","card-worklog__toggle","Work log");head.type="button";head.setAttribute("aria-expanded","false");const body=el("div","card-worklog__body");body.hidden=!worklogOpen;let loaded=false;const draw=entries=>{body.textContent="";if(!entries.length){body.appendChild(el("p","card-worklog__empty","Nothing has happened to this card yet."));return}const list=el("ol","card-worklog__list");entries.forEach(entry=>{const item=el("li","card-worklog__entry");item.appendChild(el("span","card-worklog__when",humanDate(entry.at)));item.appendChild(el("span","card-worklog__kind",entry.kind));item.appendChild(el("span","card-worklog__who",entry.who||dash));item.appendChild(el("span","card-worklog__detail",entry.detail||""));list.appendChild(item)});body.appendChild(list)};const readLog=()=>fetch("/worklog?ref="+encodeURIComponent(dialog.dataset.ref||record.ref),{cache:"no-store"}).then(r=>{if(!r.ok)throw new Error("work log failed");return r.json()}).then(draw).catch(()=>{loaded=false;body.textContent="The work log could not be read."});worklogRefresh=()=>{if(loaded&&!body.hidden)readLog()};head.addEventListener("click",()=>{const open=body.hidden;body.hidden=!open;worklogOpen=open;head.setAttribute("aria-expanded",open?"true":"false");if(!open||loaded)return;loaded=true;body.textContent="reading...";readLog()});if(worklogOpen){head.setAttribute("aria-expanded","true");loaded=true;readLog()}box.appendChild(head);box.appendChild(body);host.appendChild(box)};const renderQuestions=record=>{const all=[...(record.questions||[])];if(!all.length)return;all.sort((a,b)=>questionRank(a)-questionRank(b));const host=el("div","card-questions");all.forEach(question=>{const block=el("div","card-question");const status=question.discarded_at?"discarded":question.answer?"answered":"new";block.dataset.status=status;const settled=!!(question.answer&&question.answer.mark);if(settled)block.dataset.settled="1";const head=el("div","card-question__head");head.appendChild(el("span","card-question__id",question.id));if(settled)head.appendChild(el("span","card-question__verdict",question.answer.mark==="ok"?"\u2705":"\u274c"));else head.appendChild(el("span","card-question__status",status));block.appendChild(head);block.appendChild(el("p","card-question__text",question.text));if(settled){block.appendChild(el("blockquote","card-question__answer",question.answer.text));host.appendChild(block);return}if(question.reason)block.appendChild(el("p","card-question__reason","Why: "+question.reason));const fileList=(files,label)=>{if(!files||!files.length)return;const box=el("div","card-question__files");box.appendChild(el("span","card-question__files-label",label));files.forEach(file=>{const link=document.createElement("a");link.className="card-question__file";link.target="_blank";link.href=attachmentUrl(file.sha,file.extension);link.textContent=file.original_filename||(file.sha.slice(0,8)+"."+file.extension);box.appendChild(link)});block.appendChild(box)};fileList(question.attachments,"Asked with:");fileList(question.answer&&question.answer.attachments,"Answered with:");if(!question.discarded_at){const drop=el("div","card-question__drop","Drop a file here to attach it");drop.addEventListener("dragover",event=>{event.preventDefault();drop.classList.add("is-over")});drop.addEventListener("dragleave",()=>drop.classList.remove("is-over"));drop.addEventListener("drop",event=>{event.preventDefault();drop.classList.remove("is-over");[...(event.dataTransfer&&event.dataTransfer.files||[])].forEach(file=>{const reader=new FileReader();reader.onload=()=>mutate("/question/attach",{id:question.id,filename:file.name,to:question.answer?"answer":"question",content_base64:String(reader.result).split(",")[1]||""});reader.readAsDataURL(file)})});block.appendChild(drop);}if(question.voice){const play=el("button","card-question__play","\u25b6 Play the question");play.type="button";const audio=document.createElement("audio");audio.controls=true;audio.hidden=true;audio.className="card-question__audio";audio.src=attachmentUrl(question.voice.sha,question.voice.extension);play.onclick=()=>{audio.hidden=false;play.hidden=true;audio.play().catch(()=>{})};block.append(play,audio)}const answerWith=text=>mutate("/question/answer",{id:question.id,text:text});const box=document.createElement("textarea");box.className="card-question__box";box.rows=3;box.value=question.answer?question.answer.text:"";const grow=()=>{box.style.height="auto";box.style.height=Math.min(box.scrollHeight,420)+"px"};box.addEventListener("input",grow);setTimeout(grow,0);box.placeholder=question.answer?"Edit this answer":"Type another answer";const send=el("button","card-question__send",question.answer?"Save answer":"Answer");send.type="button";send.onclick=()=>{if(box.value)answerWith(box.value)};if(status!=="discarded"&&(question.options||[]).length){const choices=el("div","card-question__choices");question.options.forEach(choice=>{const pick=el("button","card-question__choice",choice);pick.type="button";if(question.answer&&question.answer.text===choice)pick.classList.add("is-chosen");pick.onclick=()=>answerWith(choice);choices.appendChild(pick)});const other=el("button","card-question__choice card-question__other","Other\u2026");other.type="button";other.onclick=()=>{typed.hidden=false;box.focus()};choices.appendChild(other);block.appendChild(choices)}if(question.answer){block.appendChild(el("blockquote","card-question__answer",question.answer.text));block.appendChild(el("p","card-question__state",(question.answer.mark?"marked "+question.answer.mark:"not yet marked")+(question.answer.read_at?", read":", not yet read")));const marks=el("div","card-question__marks");[["ok","This settles it"],["not-ok","This does not settle it"]].forEach(pair=>{const button=el("button","card-question__mark",pair[1]);button.type="button";if(question.answer.mark===pair[0])button.classList.add("is-active");button.onclick=()=>mutate("/question/mark",{id:question.id,mark:pair[0]});marks.appendChild(button)});block.appendChild(marks)}const typed=el("div","card-question__typed");typed.append(box,send);typed.hidden=status==="discarded"||(!question.answer&&(question.options||[]).length>0);if(status!=="discarded")block.appendChild(typed);if(question.discarded_at)block.appendChild(el("p","card-question__state","Set aside "+question.discarded_at));host.appendChild(block)});const details=sectionsHost.querySelector('[data-section="details"]');const comments=sectionsHost.querySelector('[data-section="comments"]');sectionsHost.insertBefore(section("Questions",host),(details&&details.nextSibling)||comments||null)};const showCard=record=>{if(!record)return;showError("");renderCard(record);renderQuestions(record);renderPoliceLog(record);renderWorkLog(record);if(!dialog.open)dialog.showModal()};let cardNavStack=[];const backButton=dialog.querySelector(".card-dialog__back");const updateBackButton=()=>{backButton.hidden=cardNavStack.length===0};const fetchRecord=(type,ref)=>fetch("/record?type="+encodeURIComponent(type)+"&ref="+encodeURIComponent(ref),{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("linked record failed");return response.json()});const navigateToCard=ref=>{const current=dialog.dataset.ref&&dialog.dataset.type?{ref:dialog.dataset.ref,type:dialog.dataset.type}:null;fetchRecord(dialog.dataset.type,ref).then(record=>{if(!record)return;if(current)cardNavStack.push(current);showCard(record);updateBackButton()}).catch(()=>{})};backButton.addEventListener("click",()=>{const previous=cardNavStack.pop();updateBackButton();if(!previous)return;fetchRecord(previous.type,previous.ref).then(record=>{if(record)showCard(record)}).catch(()=>{})});const newCardField=(label,control)=>{const row=el("div","card-new__row");row.appendChild(el("label","card-new__label",label));row.appendChild(control);return row};const openNewCard=(type,column)=>{dialog.dataset.ref="";dialog.dataset.type=type;dialog.dataset.newColumn=column;lastDialogRecordJson="";showError("");const refLine=dialog.querySelector(".card-dialog__ref");refLine.replaceChildren(el("span","","New "+type+" \u00b7 "+column+" \u00b7 reference assigned on save"));dialog.querySelector("h2").replaceChildren(el("span","","New card"));const form=el("form","card-new");form.noValidate=true;const title=el("input","card-edit-input");title.type="text";title.required=true;title.name="title";title.placeholder="Title (required)";const description=el("textarea","card-edit-input");description.rows=4;description.name="description";const priority=el("select","card-edit-input");priority.name="priority";["","1","2","3","4","5"].forEach(value=>{const option=el("option","",value===""?dash:value+" "+priorityLabels[value]);option.value=value;priority.appendChild(option)});const assignee=el("select","card-edit-input");assignee.name="assignee";const noone=el("option","",dash);noone.value="";assignee.appendChild(noone);people.forEach(person=>{const option=el("option","",person.name);option.value=person.id;assignee.appendChild(option)});form.append(newCardField("Title",title),newCardField("Description",description),newCardField("Priority",priority),newCardField("Assignee",assignee));const submit=el("button","card-edit-save","Create card");submit.type="submit";submit.dataset.createCard=column;const cancel=el("button","card-edit-cancel","Cancel");cancel.type="button";cancel.onclick=()=>dialog.close();const actions=el("div","card-new__actions");actions.append(submit,cancel);form.appendChild(actions);form.onsubmit=event=>{event.preventDefault();if(!title.value.trim()){showError("A title is required");title.focus();return}fetch("/create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:type,column:column,title:title.value,description:description.value,priority:priority.value,assignee:assignee.value})}).then(response=>response.json()).then(result=>{window.__tiraLastMutation="/create";window.__tiraMutationSeq=(window.__tiraMutationSeq||0)+1;if(!result.ok){showError(result.error||"Unable to create the card");return}dialog.dataset.ref=result.record.ref;delete dialog.dataset.newColumn;showError("");renderCard(result.record);refreshDashboard()}).catch(()=>{window.__tiraMutationSeq=(window.__tiraMutationSeq||0)+1;showError("Unable to create the card")})};sectionsHost.replaceChildren(section("New card",form));if(!dialog.open)dialog.showModal();title.focus()};dialog.querySelector(".card-dialog__close").addEventListener("click",()=>dialog.close());const proofDialog=document.querySelector(".proof-dialog");proofDialog.querySelector(".proof-dialog__close").addEventListener("click",()=>proofDialog.close());const buildCard=record=>{const item=document.createElement("li");item.dataset.ref=record.ref;item.dataset.mtime=String(((record._mtime||0)*1000)||Date.parse(record.updated_at||record.last_updated||"")||0);item.dataset.priority=record.priority===undefined||record.priority===null?"":String(record.priority);const button=document.createElement("button");button.className="card"+(record.waiting?" card--waiting":"")+(record.to_review?" card--to-review":"");button.type="button";button.dataset.ref=record.ref;const ref=document.createElement("span");ref.className="card__ref";ref.textContent=record.ref;button.appendChild(ref);if(showTitles){const title=document.createElement("span");title.className="card__title";title.textContent=record.title||"";button.appendChild(title)}item.appendChild(button);return item};const updateBoards=data=>{recordsByRef.clear();document.querySelectorAll(".board").forEach(board=>{const type=board.dataset.type;(data._column_order?.[type]||[]).forEach(column=>{const list=[...board.querySelectorAll(".cards")].find(node=>node.dataset.column===column);if(!list)return;const records=data[type]?.[column]||[];records.forEach(record=>recordsByRef.set(record.ref,record));list.replaceChildren(...records.map(buildCard))});sortBoard(board,document.documentElement.dataset.sort)});bindBoards();updateColumnCounts();markSelection()};}
    ...
    my $dialog = $args{live}
      ? '<dialog class="card-dialog"><header><button class="card-dialog__back" type="button" aria-label="Back" hidden>&larr;</button><div><span class="card-dialog__ref">Card</span><h2 class="card-dialog__title">Details</h2></div><button class="card-dialog__close" type="button" aria-label="Close" autofocus>&times;</button></header><p class="card-dialog__error" hidden></p><div class="card-viewer" hidden><header><button class="card-viewer__prev" type="button" aria-label="Previous attachment">&larr;</button><span class="card-viewer__name"></span><button class="card-viewer__next" type="button" aria-label="Next attachment">&rarr;</button><a class="card-viewer__download" target="_blank">Download</a><button class="card-viewer__close" type="button" aria-label="Close viewer">&times;</button></header><img hidden alt="attachment preview"><iframe hidden title="attachment preview"></iframe><pre class="card-viewer__text" hidden></pre><video class="card-viewer__video" hidden controls playsinline></video><audio class="card-viewer__audio" hidden controls></audio><div class="card-viewer__fallback" hidden><p>Preview is not supported for this file in this browser.</p><p>Use the Download button above to open it locally.</p></div></div><div class="card-dialog__sections"></div></dialog>'
        ...
        . '<dialog class="policy-dialog"><header><h2>Policies</h2>'
        . '<button class="policy-dialog__close" type="button" aria-label="Close" autofocus>&times;</button></header>'
        . '<p class="policy-dialog__error" hidden></p>'
        . '<div class="policy-dialog__tabs">'
        . '<button type="button" class="policy-dialog__tab is-active" data-policy-tab="declared">Declared</button>'
        . '<button type="button" class="policy-dialog__tab" data-policy-tab="declined">Declined</button>'
        . '<button type="button" class="policy-dialog__tab" data-policy-tab="undeclared">Undeclared</button>'
        . '</div>'
        . '<ul class="policy-declared" data-policy-pane="declared"></ul>'
        . '<ul class="policy-declined" data-policy-pane="declined" hidden></ul>'
        . '<ul class="policy-undeclared" data-policy-pane="undeclared" hidden></ul>'
        . '<form class="policy-form"><h3>Declare / edit a policy</h3>'
        . '<div class="policy-form__row"><label>Rule</label><select class="policy-form__rule"></select></div>'
        . '<div class="policy-form__row"><label>Action</label><select class="policy-form__action"></select></div>'
        . join( '', map {
            my $field = $_;
            my $badge = $field eq 'message'
              ? ' <button type="button" class="policy-form__token-hint" data-token-hint aria-label="Which {token}s can I use here?">?</button>'
              : '';
            '<div class="policy-form__row" data-field="' . $field . '"><label><span class="policy-form__label-text">' . $field . '</span>' . $badge . '</label><input type="text" class="policy-form__' . $field . '"></div>';
        } qw(enter before column age read_age max pattern message require sandbox require_link link_to) )
        . '<div class="policy-form__token-popover" data-token-popover hidden></div>'
        . '<div class="policy-form__row" data-field="type"><label>Scope: type</label>'
        . '<select class="policy-form__type"><option value="">(whole project)</option><option value="sow">sow</option><option value="epic">epic</option><option value="ticket">ticket</option></select></div>'
        . '<div class="policy-form__row" data-field="on_column"><label>Scope: on column</label><input type="text" class="policy-form__on_column"></div>'
        . '<div class="policy-form__row" data-field="ref"><label>Scope: ref</label><input type="text" class="policy-form__ref"></div>'
        . '<div class="policy-form__row" data-field="reason"><label>Decline reason</label><input type="text" class="policy-form__reason"></div>'
        . '<footer><button type="button" class="policy-form__clear">New</button>'
        . '<button type="button" class="policy-form__remove" hidden>Remove</button>'
        . '<button type="button" class="policy-form__decline">Decline</button>'
        . '<button type="button" class="policy-form__declare">Declare</button></footer></form></dialog>'
      : '';
...
.board--tasklist{--accent:#818cf8;--task-pending:#fbbf24;--task-working:#818cf8;--task-done:#34d399}
.tasklist-controls{display:flex;flex-wrap:wrap;gap:.5rem;align-items:center;padding:.4rem .45rem 1rem}
.tasklist-controls input[type="text"]{flex:1;min-width:10rem;padding:.5rem .7rem;border:1px solid var(--line);border-radius:.6rem;background:rgba(2,6,23,.5);color:var(--ink);font:inherit}
.tasklist-controls button{padding:.48rem .68rem;color:var(--muted);background:transparent;border:1px solid var(--line);border-radius:.55rem;font-size:.72rem;font-weight:750;cursor:pointer}
.tasklist-controls button:hover{color:var(--ink);border-color:var(--accent)}
.tasklist-session{padding:.4rem .6rem;border:1px solid var(--line);border-radius:.55rem;background:rgba(2,6,23,.5);color:var(--ink);font:inherit;margin-left:auto}
.tasklist-error{color:#fca5a5;margin:0 .45rem .6rem}.tasklist-error[hidden]{display:none}
.tasklist-cards{display:grid;grid-template-columns:repeat(auto-fill,minmax(14rem,1fr));gap:.8rem;list-style:none;margin:0;padding:.4rem .45rem 1.2rem}
.tasklist-card{padding:.9rem;border-radius:.6rem .6rem 1.4rem .6rem;color:#241d06;box-shadow:0 12px 26px rgba(0,0,0,.32),0 1px 0 rgba(255,255,255,.35) inset;transform:rotate(-1deg);font-size:.86rem;line-height:1.4;display:flex;flex-direction:column;gap:.5rem;transition:transform .15s ease}.tasklist-card.is-dragover{outline:2px dashed var(--accent,#818cf8);outline-offset:2px}
.tasklist-card:nth-child(even){transform:rotate(1deg)}
.tasklist-card:hover{transform:rotate(0deg) translateY(-2px)}
.tasklist-card[data-status="0"]{background:var(--task-pending)}
.tasklist-card[data-status="1"]{background:var(--task-working);color:#f5f3ff}
.tasklist-card[data-status="2"]{background:var(--task-done);color:#052e1d}
.tasklist-card__id{font:800 .7rem/1 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.06em;opacity:.75}
.tasklist-card__text{font-weight:650;overflow-wrap:anywhere}.tasklist-card__text-input{width:100%;resize:none;overflow:hidden;font:inherit;font-weight:650;padding:0;border:0;background:transparent;color:inherit}
.tasklist-card__row{display:flex;flex-wrap:wrap;gap:.35rem;align-items:center}
.tasklist-card select,.tasklist-card input,.tasklist-card button{padding:.3rem .5rem;border:1px solid rgba(0,0,0,.25);border-radius:.5rem;background:rgba(255,255,255,.4);color:inherit;font:inherit;cursor:pointer;min-width:0}
.tasklist-card input[type="text"]{cursor:text;width:6rem}
.tasklist-card input[type="file"]{cursor:pointer;flex:1;min-width:6rem}
.tasklist-card__chips{display:flex;flex-wrap:wrap;gap:.3rem}
.tasklist-card__chip{display:inline-flex;align-items:center;gap:.3rem;padding:.15rem .4rem;border-radius:.4rem;background:rgba(255,255,255,.4);font-size:.72rem;font-weight:700}.tasklist-card__chip-link{cursor:pointer;text-decoration:underline dotted}.tasklist-card__chip-link:hover{text-decoration-style:solid}.tasklist-card__ref-suggestions{position:absolute;top:100%;left:0;right:0;z-index:5;display:flex;flex-direction:column;background:#fff;border:1px solid rgba(0,0,0,.25);border-radius:.4rem;box-shadow:0 6px 16px rgba(0,0,0,.25);overflow:hidden}.tasklist-card__ref-suggestion{display:block;width:100%;text-align:left;padding:.3rem .5rem;border:0;background:transparent;color:#241d06;font-size:.78rem;cursor:pointer}.tasklist-card__ref-suggestion:hover{background:rgba(0,0,0,.08)}
.tasklist-card__chip button{padding:0 .3rem;border:0;background:transparent;font-weight:800;line-height:1.4}
...
.card-edit{display:inline-flex;align-items:center;gap:.45rem;flex-wrap:wrap}.card-value--long{display:block;width:100%}.card-md-body{display:block}.card-md-body .card-md-p{margin:0 0 .5rem}.card-md-body .card-md-p:last-child{margin-bottom:0}.card-edit--long{display:flex;width:100%;align-items:flex-start;flex-wrap:wrap;gap:.45rem}.card-edit--long .card-edit-input{flex:1 1 100%;order:2}.card-edit--long .card-md-bar{flex:1 1 100%;order:1}.card-edit--long .card-edit-save,.card-edit--long .card-edit-cancel{order:3}.card-edit-input{min-width:12rem;padding:.45rem .6rem;color:var(--ink);background:rgba(2,6,23,.6);border:1px solid rgba(103,232,249,.35);border-radius:.6rem;font:inherit;font-size:.86rem}textarea.card-edit-input{width:100%;min-width:16rem}.card-edit-save,.card-edit-cancel,.card-comment__action{padding:.4rem .7rem;color:#07111f;background:#67e8f9;border:0;border-radius:.55rem;font-size:.74rem;font-weight:750;cursor:pointer}.card-edit-cancel,.card-comment__action{color:var(--ink);background:rgba(255,255,255,.1);border:1px solid var(--line)}.card-comment__action--danger:hover{color:#fecaca;border-color:rgba(248,113,113,.5);background:rgba(153,27,27,.3)}
...
.card-viewer{position:absolute;inset:0;z-index:5;display:flex;flex-direction:column;background:rgba(2,6,23,.96);border-radius:1.4rem}.card-viewer[hidden]{display:none}.card-viewer header{display:flex;align-items:center;gap:1rem;padding:1rem 1.4rem;border-bottom:1px solid var(--line)}.card-viewer__name{color:#a5f3fc;font:750 .8rem/1 ui-monospace,SFMono-Regular,Menlo,monospace;margin-right:auto}.card-viewer__download{color:#07111f;background:#67e8f9;padding:.4rem .8rem;border-radius:.55rem;font-size:.74rem;font-weight:800;text-decoration:none}.card-viewer__close{color:var(--ink);background:rgba(255,255,255,.08);border:1px solid var(--line);border-radius:.7rem;font-size:1.2rem;cursor:pointer}.card-viewer__prev,.card-viewer__next{color:var(--ink);background:rgba(255,255,255,.08);border:1px solid var(--line);border-radius:.7rem;font-size:1.1rem;cursor:pointer;padding:.3rem .7rem}.card-viewer__prev[disabled],.card-viewer__next[disabled]{opacity:.35;cursor:default}.card-viewer img{max-width:100%;max-height:100%;margin:auto;object-fit:contain}.card-viewer iframe{flex:1;width:100%;border:0;background:#f8fafc}.card-viewer__text{flex:1;overflow:auto;margin:0;padding:1.2rem 1.4rem;color:#dbeafe;background:transparent;font:500 .82rem/1.65 ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap;word-break:break-word}.card-viewer__text .tok--comment{color:#64748b;font-style:italic}.card-viewer__text .tok--string{color:#86efac}.card-viewer__text .tok--number{color:#fbbf24}.card-viewer__text .tok--keyword{color:#93c5fd;font-weight:600}.card-viewer__text[hidden]{display:none}.card-viewer__video{flex:1;width:100%;max-height:100%;background:#000}.card-viewer__audio{width:calc(100% - 2.8rem);margin:2rem 1.4rem}.card-viewer__fallback{margin:auto;padding:2rem;text-align:center;color:var(--muted);font-size:.92rem;line-height:1.7}.card-viewer__video[hidden],.card-viewer__audio[hidden],.card-viewer__fallback[hidden]{display:none}
...
.board-policies{margin-left:.3rem;padding:.48rem .68rem;color:var(--muted);background:transparent;border:1px solid var(--line);border-radius:.55rem;font-size:.72rem;font-weight:750;cursor:pointer}.board-policies:hover{color:var(--ink)}.policy-dialog{width:min(50rem,92vw);max-height:88vh;overflow:auto;border:1px solid var(--line);border-radius:1rem;background:var(--panel);color:var(--ink);padding:1.2rem}.policy-dialog::backdrop{background:rgba(2,6,23,.72)}.policy-dialog header{display:flex;align-items:center;justify-content:space-between;margin-bottom:.8rem}.policy-dialog h2{margin:0;font-size:1rem}.policy-dialog__error{color:#fca5a5;margin:0 0 .6rem}.policy-dialog__close{border:1px solid var(--line);border-radius:.5rem;background:transparent;color:var(--muted);cursor:pointer;padding:.35rem .6rem;font:inherit}.policy-dialog__tabs{display:flex;gap:.4rem;margin-bottom:.7rem}.policy-dialog__tab{border:1px solid var(--line);border-radius:.5rem;background:transparent;color:var(--muted);cursor:pointer;padding:.4rem .7rem;font:inherit}.policy-dialog__tab.is-active{color:var(--ink);border-color:var(--accent,#818cf8);font-weight:700}.policy-declared,.policy-declined,.policy-undeclared{list-style:none;margin:0 0 1rem;padding:0;display:flex;flex-direction:column;gap:.4rem}.policy-declared[hidden],.policy-declined[hidden],.policy-undeclared[hidden]{display:none}.policy-row{display:flex;flex-wrap:wrap;align-items:center;gap:.5rem;padding:.45rem .6rem;border:1px solid var(--line);border-radius:.6rem;background:rgba(2,6,23,.32);font-size:.84rem}.policy-inline-capture{display:inline-flex;gap:.3rem;align-items:center}.policy-row__text{flex:1;min-width:0;overflow-wrap:anywhere}.policy-row__header{font-weight:700}.policy-row__params{color:var(--muted);font-size:.82rem;margin-top:.2rem}.policy-row button{border:1px solid var(--line);border-radius:.5rem;background:transparent;color:var(--muted);cursor:pointer;padding:.3rem .6rem;font:inherit;margin-left:auto}.policy-row button:hover{color:var(--ink);border-color:var(--accent,#818cf8)}.policy-form{border-top:1px solid var(--line);padding-top:.9rem;display:flex;flex-direction:column;gap:.5rem}.policy-form h3{margin:0 0 .3rem;font-size:.9rem}.policy-form__row{display:flex;align-items:center;gap:.6rem}.policy-form__row label{min-width:9rem;color:var(--muted);font-size:.78rem}.policy-form__row input,.policy-form__row select{flex:1;padding:.4rem .55rem;border:1px solid var(--line);border-radius:.5rem;background:rgba(2,6,23,.5);color:var(--ink);font:inherit}.policy-form__row[hidden]{display:none}.policy-form__token-hint{width:1.3rem;height:1.3rem;padding:0;margin-left:.35rem;border:1px solid var(--line);border-radius:50%;background:transparent;color:var(--muted);cursor:pointer;font-size:.68rem;line-height:1}.policy-form__token-hint:hover{color:var(--ink);border-color:var(--accent,#818cf8)}.policy-form__token-popover{margin-left:9.6rem;padding:.5rem .6rem;border:1px solid var(--line);border-radius:.5rem;background:rgba(2,6,23,.5);color:var(--muted);font-size:.78rem;display:flex;flex-direction:column;gap:.15rem}.policy-form__token-popover[hidden]{display:none}.policy-form footer{display:flex;justify-content:flex-end;gap:.5rem;margin-top:.6rem}.policy-form footer button{border:1px solid var(--line);border-radius:.5rem;background:transparent;color:var(--muted);cursor:pointer;padding:.4rem .7rem;font:inherit}.policy-form__declare{color:var(--ink);font-weight:700}.policy-form footer button:hover{color:var(--ink);border-color:var(--accent,#818cf8)}
CSS
...
the one function both C<checklist_update> and C<required_item_update> call - so
what it refuses, both families refuse, and what it changed in 4.64, both
families gained.

Marking done still costs a matching pair. What is new is that a C<--command>
may arrive ahead of its proof: it is recorded, the item's status is untouched,
and the proof follows later repeating its command.

It arrives WITH a C<--status> or an C<--item> rather than entirely alone.
C<required_item_update> and C<checklist_update> both refuse a call that changes
nothing - "Required item or status is required" - and that guard is older than
this feature and is not relaxed by it. So the shape is
C<--status pending --command TEXT> on an item already pending, which reads
oddly and is honest: the status is being restated, not changed. Until TKT-628 this returned C<undef> for
any status that was not C<done>, so a lone command was parsed by the CLI,
accepted, and dropped here before the pair logic was reached - the call
succeeded, nothing was stored, and the caller was told nothing.

The remaining early return is about having nothing to store rather than about
which status was named, which is the distinction that makes an ordinary status
change still free of any evidence requirement.

Two refusals fire on different shapes and it is worth knowing which: a side
missing entirely gives "requires at least one --command/--proof pair", while an
unequal COUNT of the two gives "Every --command needs a matching --proof". A
lone C<--command> has no proofs at all, so it meets the first - a test written
against the second passes for the wrong reason, which is a mistake this
project has already made once.

The refusal for an empty half forks on WHICH HALF is empty rather than on the
status: "The pair is what says the work happened" for a done claim, "An empty
command announces nothing" only when there is no proof side at all, and "An
empty half is not evidence" when a blank proof was supplied beside a real
command. Forking on the status alone told a caller with a blank C<--proof> that
an empty command announces nothing, naming the half that was fine.

Three further things this function and its callers had to learn once a
non-proof could flow through them, each found by a code review rather than by a
test:

C<_refuse_reused_proof> filters announcements out before it signatures
anything. It treats a missing proof as the empty string, so two items
announcing the same command hashed identically and the second was charged with
reusing evidence that did not exist. The rule is about one PROOF being passed
off as proving two things; a command with no proof yet proves nothing.

An announcement must not overwrite evidence behind a done claim. The assignment
C<$entry-E<gt>{proof} = $proof_entries> is unconditional and an announcement is
truthy, so renaming a done item while announcing replaced its proof with a
command-only entry and left the status C<done> - the state TKT-583 and TKT-585
exist to prevent, reached from a new direction. Announcing over a done item is
now refused unless C<--status> re-opens it deliberately.

Both that guard and the gate-log condition read the RESULTING status -
C<defined $args{status} ? $args{status} : $entry-E<gt>{status}> - not what the
caller passed. Keyed off the argument alone, a done item given fresh evidence
without repeating C<--status> stored it and logged nothing. And an announcement writes no C<gate_passing_log> entry - those
read "Marked ... done", and an announcement is not that. Both callers of
C<_log_proof_gate> test the status for exactly this reason; without it,
announcing a command wrote a false done-claim into the record the board keeps
...
Returns the actions a policy may take: bridge-reminder, print-reminder, log-only.
...
=head2 policy_rule_specs

Returns which parameters each policy rule needs or forbids, the same data policy_add already consults.

=head2 policy_message_fields

Returns the sorted list of C<{token}> names a policy's C<--message> can substitute (ref, rule, policy, detail, column, title, assignee, reporter, age, max) - the keys of the same table C<_policy_message> reads.

=head2 policy_message_field_help

Returns a one-line description for each C<policy_message_fields> token, matching docs/POLICIES.md's own "Saying it in your own words" table. TKT-519: exposed through the C<policies> browser provider so the dialog can show which tokens exist without a second, hand-copied explanation to drift.
...
=head2 _attachment_content_type

Decides what an attachment is, so that exactly one thing decides it.

Two named lists come first, for the cases where guessing would be worse than
knowing in each direction: a C<.pl> file is Perl whatever its first bytes look
like, and an empty one is still text; a C<.zip> whose first bytes happen to
read as printable must never be offered as text. Everything else is decided by
looking - C<_attachment_head> reads the first 8KB and C<_looks_like_text> calls
it binary on a NUL byte, or when more than a tenth of those bytes fall outside
the printable and common-whitespace range.

Nothing to examine is a refusal rather than a guess: no content, an empty file
and a missing path all come back as C<application/octet-stream>, because that
is where offering the download is the honest answer.

The sniff is lazy - only an extension in neither list is read - so the cost
lands on the unusual file rather than on every listing.

This exists as one function because the fault it replaces was two. The browser
viewer kept its own extension allowlist while the engine computed a type, and
the two disagreed about every source file on the board; the same shape as
TKT-224, TKT-671, TKT-681 and TKT-657, each of which turned out to be one
question with two answers. TKT-645.
...
The browser dashboard's own behaviour is built from the JavaScript this module
inlines, so a few decisions live here rather than in a file of their own.

Three helpers sit beside C<el()>. C<growBox> follows the content to a ceiling
and then scrolls; C<mdBarFor> builds a formatting bar bound to whichever
textarea it is given; C<pasteImageInto> attaches a pasted image and leaves an
C<[image: name]> marker at the caret.

Every text editor in the card dialog uses the first two, which is what stops
them drifting apart again - including the comment composer, whose inline bar
C<mdBarFor> was extracted from and which now calls it rather than keeping a
second copy. C<pasteImageInto> is wired to the long fields only, which is where
paste was asked for.

A fifth arrived with TKT-628 in 4.64. C<isAnnounced> answers whether an item
has said what it is running without having finished - a proof list carrying a
command and no proof - and the icon became four states from three:

    exempt ? mark : isDone ? tick : isAnnounced ? clock : empty box

The ordering is the whole of it. Exempt wins over everything, done over
announced, announced over untouched; a predicate inserted at the wrong point in
that chain would let a finished item read as merely started, or an exempt one
as either. C<t/playwright/announced-command.js> drives all four on one card and
asserts they render as four DISTINCT marks, because a renderer that collapsed
two onto one glyph would satisfy every individual check if the glyph happened
to be right for both.

An announced item is still offered its checkbox. It has started, not finished,
and if the checkbox went away the clock would have quietly become a second way
of saying done.

A fourth sits with them since 4.63, for the same reason and after the same
mistake. C<isDone> answers whether a required action is finished, and all three
places that ask - the done/total count, the tick-or-empty-box icon, and whether
an actionable checkbox is drawn - go through it rather than each testing
C<entry.status === "done"> for themselves. They did test it for themselves, and
one of them was enough to make the page disagree with the engine: an item stored
as C<Done> read finished to every gate and outstanding to the dashboard, on 21
cards of one real board.

It is written to agree with the engine -
C<String((entry&&entry.status)||"").toLowerCase()==="done"> against the engine's
C<lc($_-E<gt>{status} // '')> - and it does, on every value either has been
given: a missing key, C<null>, the empty string, whitespace, a numeric zero and
the string C<"0"> all read as not-done on both sides.

They are not the same expression, which is worth knowing before someone
"simplifies" one to match the other. A numeric C<0> is discarded to C<""> by the
C<||> before C<toLowerCase> ever sees it, where Perl lowercases it to C<"0"> and
compares that; the answer is the same and the route is not. The empty-string
default IS deliberately identical, and it is what lets an item written before
the status field existed render as outstanding rather than throwing and taking
the whole dialog down with it.

The checklist renderer is NOT one of the three and must not become one. It
prints C<"[" + status + "] "> verbatim and compares nothing, which makes it
honest about whatever value it holds; C<t/417> asserts it stays that way, so a
later change that goes looking for C<status> cannot quietly correct a renderer
that was already right. TKT-601.

C<growBox> defaults to 420px because two grow handlers already existed and they
differ: the tasklist card input grows without limit, the question answer box
stops at 420. A description runs to paragraphs, and unbounded growth pushes the
dialog's own Save and Cancel off the screen, so the answer box is the precedent
worth copying. C<overflowY> is set explicitly - hidden while it fits, auto once
capped - because a capped box that does not scroll clips instead.

C<mutate> takes an optional third argument that suppresses its reload. A paste
happens while an editor is OPEN, and reloading re-renders the dialog and takes
that editor with it: the attachment survives and the writing does not. The
attachment appears on the next reload, which is when the author saves. TKT-586.
