...
| `monitor-output` | — | a running `monitor`-kind job's own output, carried to the bridge. **His instruction, 2026-09-02 16:55**: "Monitors should not have separate logs. They all go to the policy bridge." `monitor-dead` above makes a STOPPED monitor say so; this is a RUNNING one still being unheard - a live poller can be reporting something useful while the agent, which reads the bridge, never sees it. It is why TKT-840 could not migrate the Telegram bridge onto a board-owned job: a monitor-kind job would have put those messages in a file nobody opens and silently cut the channel. **The file stays, and that is not a compromise**: TKT-842 chose one because a pipe with no reader fills at about 64KB and blocks the child forever - a monitor that has stopped working while still looking started, which is worse than the silence being fixed. So this FOLLOWS the spool rather than replacing it. **An offset, not the ledger `job-due` uses**: that announces once per due WINDOW with a `sub_key`, and a stream has no windows. The question is which BYTES have been said, and an offset answers it exactly - where a ledger keyed on text would swallow the repetitions a poller legitimately prints, which one doing the same work every minute does constantly. **The pass writes**, deliberately: the offset and the last-output timestamp advance under the same project lock as the announcement, because if announcing and advancing could come apart they eventually would, and the failure is either losing a monitor's output or repeating it forever. **Capped per pass and the cap says what it dropped** - a bridge that truncates silently is a bridge that lies, which is the failure this whole epic is about. **Silent for a cron job, a disabled monitor and one with no command**, matching `monitor-dead`'s own exemptions. **No age**, and refused rather than ignored: output that has arrived does not become more arrived by waiting. **Whole-board**, like `job-due` - a job is not about a card. The last-output timestamp is what TKT-863's heartbeat light reads, so that card needs no cooperation from any monitor beyond what it already prints. EPC-014, TKT-851. |
...
And once it is running, what it SAYS should reach the same place. A monitor
writes to a per-job spool, and before `monitor-output` that was the only place
it went — so a live poller could be reporting something useful while the agent,
which reads the bridge, never saw it. His instruction on 2026-09-02: "Monitors
should not have separate logs. They all go to the policy bridge."
...
The spool is followed rather than replaced. A pipe with no reader fills at about
64KB and blocks the monitor forever, which is a worse silence than the one being
fixed, so the file stays and the rule reads from a stored offset. Twenty lines
per pass, with the count of any it dropped in the same line — a bridge that
truncates quietly is a bridge that lies. Nothing is said for a cron job, a
disabled monitor, one with no command, or one that has said nothing new.
