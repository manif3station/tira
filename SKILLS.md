...
tira.required-action.update --ref REF (--id REQ-NNN | --ids REQ-NNN,... [--ids ...]) [--item TEXT] [--status TEXT] [--command TEXT ... [--proof TEXT ... | --proof-file PATH ...]] [--repeated-reason TEXT] [--repeated-confirm CODE] [-o FORMAT]
...
`lib/Tira.pm` is 15,023 lines now (TKT-1114, 5.142 - measured now rather
than carried forward, the fault this section is about). It was 14,996 as
of TKT-846 (5.139); TKT-1114 added `_announce_upgrade`, the project-lock
wrapper around the upgrade-announcement decision (a genuine new concern,
not decomposition). As of TKT-1102 (5.131) it was 14,866. Two drops
stacked before that:
...
**The filter went where the counting happens, not into `record_list`**, and that placement is the decision. Five call sites already pass `record_list` an `include_discard` it does not read, so teaching it to filter would silently change what every other caller gets — including `person_remove` and `link_type_remove`, which walk everything on purpose because a person named by a discarded card still has a historical reference. A read that returns everything is not the fault; a total that claims to match a view excluding set-aside work, and does not, is. The sweep for other summaries with the same gap found none: `card_holes`, `work_order` and `policy_evaluate` each exclude discard already, and the two that do not are not summaries. `record_list`'s inert flag is filed separately as TKT-970.
