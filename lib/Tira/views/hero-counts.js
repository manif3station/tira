...
if (heroCounts) {
  ...
  const refreshTaskTotal = () =>
    fetch("/tasklist", { cache: "no-store" })
      ...
      .then((list) => {
        // TKT-817: status 0/1 are pending/working, 2 is done - a done item
        // left unpruned inflated this count for no reason a reader can see.
        // Missing status defaults to pending (0), matching outstanding_summary's
        // own `$_->{status} // 0` in lib/Tira.pm.
        taskTotal = Array.isArray(list)
          ? list.reduce((n, item) => {
              const status = item.status ?? 0;
              return status === 0 || status === 1 ? n + 1 : n;
            }, 0)
          : 0;
        paintHeroCounts();
      })
...
if (heroEl) {
  // TKT-955: a single threshold at 24px fed back into itself - compacting
  // shortens the page, which can put scrollY back under 24, which expands it
  // again, in the same scroll gesture. Two thresholds instead of one:
  // compact only above the high one, expand only below the low one, with a
  // band between them wider than the height the compact/full switch can
  // possibly move the page (his screen recording showed the shake confined
  // to a few pixels either side of 24, so 64px of separation is well clear
  // of it). Once compact, staying compact anywhere from 8px to the top of
  // the page is what breaks the loop - the state can no longer put itself
  // back on the other side of the threshold that set it.
  const HERO_COMPACT_AT = 64;
  const HERO_EXPAND_AT = 8;
  const applyHeroScroll = () => {
    const y = window.scrollY;
    if (y > HERO_COMPACT_AT) heroEl.classList.add("hero--compact");
    else if (y < HERO_EXPAND_AT) heroEl.classList.remove("hero--compact");
  };
