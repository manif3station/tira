...
more than zero."
Since 5.90, `tasklist.list` also returns `status_name` (pending, working,
or done) alongside the stored status code, so a reader is not left
decoding a bare 0/1/2.
...
The board-wide police policy engine — the 49 rules police itself watches,
...
Since 5.50 that covers all of cron: hour steps, ranges and lists, named weekdays,
days of the month and named months, and **nothing is left as raw cron**. Anything
it cannot say exactly is marked and says why — *About every 7 minutes (restarts
each hour)*. What is exact is said plainly, so `*/60` reads *Every hour, on the
...
2,719 lines now, with the bodies in `lib/Tira/CLI/` - `Browser`, `Police`, `Serve`,
...
warning log, TKT-1102). `lib/Tira.pm` is 15,896 lines now (TKT-1144 lifted the required-action check into `record_move` itself); it was 15,814 lines as of TKT-1118, which added the enforcement-lock coverage for `_announce_upgrade`, `bridge_write`, `bridge_touch`, `_enforcement_record`, `rule_suspend`, `police_suspend`; it was 15,584 lines as of TKT-1133, and 15,368 lines as of TKT-972. TKT-1098 first
...
covers `49 rules cover`, `49 rules police`, `49 police
rules` and `49 policy rules`, and it is the whole of its reach, worth stating
