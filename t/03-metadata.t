...
close $env;
like( $env_text, qr/^VERSION=0\.82$/m, '.env stores version 0.82' );
...
is( scalar @use_cases, 104, 'SKILLS.md contains exactly 104 numbered use cases' );
my %seen;
while ( $skills_text =~ /^### UC-(\d{3}):/mg ) {
    $seen{$1}++;
}
is_deeply( [ sort keys %seen ], [ map { sprintf '%03d', $_ } 1 .. 104 ], 'use cases are numbered UC-001 through UC-104' );
...
use lib 'lib';
use Tira;
is( $Tira::VERSION, '0.82', 'module version matches .env' );
