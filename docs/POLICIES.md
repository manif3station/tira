...
| `card-duration` | `--column --age` | a card sitting in one place too long. `--age` was otherwise a guess, picked by hand and never checked against what the board itself records - `tira.dwell.report` reads back the real median/p90/max seconds cards spend in each column, from every card's own recorded moves, so the threshold can be a reading instead. TKT-366. **A column that holds containers needs its own reading.** A SOW or an epic legitimately sits in a working column for as long as its children take, so a threshold set for work items reports it forever and cannot be settled by anything an agent does - the only settling move is finishing every child. Measured on this board: `in-progress` holds the SOW and the epic and no tickets at all, with a median dwell of 4h31m but a p90 of 57h09m and a max of 5d08h, against a policy of 8h. The result was two CRITICAL reminders that fired 63 and 69 times over three days with no action available, which is worse than silence: a signal nobody can act on teaches the reader to skip the channel it arrives on. Re-thresholded to 58h from the p90 (TKT-573), both settled on the first pass. Note the distinction from `card-still` when diagnosing this: `card-duration` measures TIME IN A COLUMN, so updating a parent - folding a child's work back into it, adding comments or key details - does not settle it and never could. Only moving the card does. |
