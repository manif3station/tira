...
sub question_voice {
    ...
    if ( !$args{remove} ) {
        ...
        my $safe = $self->_safe_path_input( $path, 'voice note' );
        open my $fh, '<:raw', $safe or die "Cannot read voice note '$path': $!\n";
        my $content = do { local $/; <$fh> };
        close $fh;
        die "A voice note cannot be empty\n" if !length $content;
        die "That voice note is too large (16 MB maximum)\n" if length($content) > 16 * 1024 * 1024;

        # The same content-addressed store every attachment uses, so the same
        # recording on ten questions is one file, and the route that already
        # serves attachments serves this with nothing new to secure.
        my $sha = sha256_hex($content);
        $sha =~ /\A([0-9a-f]{64})\z/ or die "Cannot validate voice note\n";
        $sha = $1;
        ...
        $reference = {
            ...
            original_filename => basename($safe), added_at => $self->{clock}->(),
        };
    }
...
sub _dashboard_table {
    ...
    for my $type (qw(sow epic ticket)) {
        ...
        my $cells = join '', map {
            ...
            '<section class="column"><h3 class="column__head"><span class="column__name">' . $slug
