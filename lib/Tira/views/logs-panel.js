...
if (logsSection) {
  ...
  const logsClear = logsSection.querySelector(".logs-clear");

  // TKT-1020. This clears the current rendering only - loadLogs still polls
  // every five seconds, so a cleared panel is "read it clean right now" and
  // will show new requests as they arrive, not a retention change.
  if (logsClear) {
    logsClear.addEventListener("click", () => {
      logsList.replaceChildren();
      logsNote.textContent = "Cleared. Waiting for the next request.";
    });
  }
