...
close $env;
like( $env_text, qr/^VERSION=1\.32$/m, '.env stores version 1.32' );
...
is( scalar @use_cases, 129, 'SKILLS.md contains exactly 129 numbered use cases' );
my %seen;
while ( $skills_text =~ /^### UC-(\d{3}):/mg ) {
    $seen{$1}++;
}
is_deeply( [ sort keys %seen ], [ map { sprintf '%03d', $_ } 1 .. 129 ], 'use cases are numbered UC-001 through UC-129' );
...
use lib 'lib';
use Tira;
is( $Tira::VERSION, '1.32', 'module version matches .env' );
...
is( scalar @commands, 132, 'release ships exactly 132 executable CLI entrypoints' );
