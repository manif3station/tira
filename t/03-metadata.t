...
close $env;
like( $env_text, qr/^VERSION=0\.81$/m, '.env stores version 0.81' );
...
is( scalar @use_cases, 103, 'SKILLS.md contains exactly 103 numbered use cases' );
my %seen;
while ( $skills_text =~ /^### UC-(\d{3}):/mg ) {
    $seen{$1}++;
}
is_deeply( [ sort keys %seen ], [ map { sprintf '%03d', $_ } 1 .. 103 ], 'use cases are numbered UC-001 through UC-103' );
...
use lib 'lib';
use Tira;
is( $Tira::VERSION, '0.81', 'module version matches .env' );
