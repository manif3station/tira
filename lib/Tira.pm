        ...
        for my $column (@columns) {
            ...
            for my $filename ( readdir $dh ) {
                ...
                if ( $args{summary} ) {
                    ...
                    my $record = $self->_read_json($path);
                    $card = { %{$record}, column => $column->{name}, waiting => _card_waiting($record) };
                }
                $card->{_mtime} = $stat[9];
                push @cards, $card;
            }
