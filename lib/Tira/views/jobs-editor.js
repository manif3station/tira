...
if (jobsSection) {
  ...
  const openEditor = (job) => {
    ...
    let commandField = null;
    {
      ...
      commandField.value = (job && job.command) || "";
    ...
    const modeCommand = makeRadio(modeRow, modeName, "command", "Command", true);
    const modeMessage = makeRadio(modeRow, modeName, "message", "Message", false);
    ...
    save.addEventListener("click", () => {
      ...
      // send it either.
      if (!loopRow.hidden && loopBox.checked) {
        payload.restart_every = Number(loopEvery.value);
      }
      if (!expectRow.hidden && expectField.value !== "") {
        payload.expect_every = Number(expectField.value);
      }
      if (!creating) {
        payload.id = job.id;
      }
